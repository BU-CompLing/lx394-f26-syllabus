...
nav_order: 4
---
<a id="chap-structured-programming"></a>
...
1. How do the fundamental building blocks work, such as loops, functions and assignment?
1. What are some of the pitfalls with Python programming and how can you avoid them?
...
code <a href="#assignment1" id="ref-assignment1"><span><span>[1]</span></span></a>,
the value of `foo` (the string `'Monty'`) is assigned to `bar`.
That is, `bar` is a <a id="copy_index_term"></a>copy of `foo`, so when we overwrite
`foo` with a new string `'Python'` on line <a href="#assignment2" id="ref-assignment2"><span><span>[2]</span></span></a>, the value
...
<a href="#assignment3" id="ref-assignment3"><span><span>[1]</span></span></a> assigns the reference of `foo` to the new variable `bar`.
Now when we modify something inside `foo` on line <a href="#assignment4" id="ref-assignment4"><span><span>[2]</span></span></a>, we can see
...
<a id="fig-array-memory"></a>

![../images/array-memory.png](https://www.nltk.org/images/array-memory.png){: style="width: 527.25px; height: 187.5px;" }
...
The line `bar = foo` [[1]](#assignment3) does not copy the contents of the
...
In [4.1](#fig-array-memory), we see that a list `foo` is
...
parameter passing ([4.4](#sec-functions)).
...
<a id="sec-sequences"></a>

### 4.2 Sequences

So far, we have seen two kinds of sequence object: strings and lists. Another
kind of sequence is called a <a id="tuple_index_term"></a>tuple.
Tuples are formed with the comma operator <a href="#create-tuple" id="ref-create-tuple"><span><span>[1]</span></span></a>, and typically enclosed
...
of members. Like lists and strings, tuples can be indexed <a href="#index-tuple" id="ref-index-tuple"><span><span>[2]</span></span></a>
and sliced <a href="#slice-tuple" id="ref-slice-tuple"><span><span>[3]</span></span></a>, and have a length <a href="#length-tuple" id="ref-length-tuple"><span><span>[4]</span></span></a>.
...
as shown in [4.1](#tab-python-sequence).

<a id="tab-python-sequence"></a>
...
The sequence functions illustrated in [4.1](#tab-python-sequence) can be combined
...
cut the data <a href="#cut-location" id="ref-cut-location"><span><span>[1]</span></span></a>, then cut the sequence at that location <a href="#cut-sequence" id="ref-cut-sequence"><span><span>[2]</span></span></a>.
...
<a href="#cut-preserve" id="ref-cut-preserve"><span><span>[3]</span></span></a>. We can also verify that the ratio of the sizes of the two pieces is what
we intended <a href="#cut-ratio" id="ref-cut-ratio"><span><span>[4]</span></span></a>.
...
A simple string is actually an object with methods defined on it such as `split()` <a href="#string-object" id="ref-string-object"><span><span>[1]</span></span></a>.
We use a list comprehension to build a list of tuples <a href="#tuple-comprehension" id="ref-tuple-comprehension"><span><span>[2]</span></span></a>,
where each tuple consists of a number (the word length) and the
word, e.g. `(3, 'the')`. We use the `sort()` method <a href="#sort-method" id="ref-sort-method"><span><span>[3]</span></span></a>
to sort the list in-place. Finally, we discard the length
information and join the words back into a single string <a href="#discard-length" id="ref-discard-length"><span><span>[4]</span></span></a>.
(The underscore [[4]](#discard-length) is just a regular Python variable,
...
fixed length. We often use a tuple to hold a <a id="record_index_term"></a>record,
a collection of different <a id="fields_index_term"></a>fields relating to some entity.
...
lists are <a id="mutable_index_term"></a>mutable, while tuples are <a id="immutable_index_term"></a>immutable. In other
...
expression inside a call to some other function <a href="#max-comprehension" id="ref-max-comprehension"><span><span>[1]</span></span></a>,
but Python allows us to omit the brackets <a href="#max-generator" id="ref-max-generator"><span><span>[2]</span></span></a>.
...
The second line uses a <a id="generator_expression_index_term"></a>generator expression. This is more than a notational convenience:
in many language processing situations, generator expressions will be more efficient.
In [[1]](#max-comprehension), storage for the list object must be allocated
before the value of max() is computed. If the text is
very large, this could be slow. In [[2]](#max-generator), the data is streamed to the calling
...
as already explained in [3](https://www.nltk.org/book/ch02.html#sec-reusing-code).
...
function, and give it a name such as `get_text()`, as shown in [4.2](#code-get-text).

<a id="code-get-text"></a>
...
*[**Example 4.2 (code_get_text.py)**](https://www.nltk.org/book/pylisting/code_get_text.py): **Figure 4.2**: Read text from a file*
...
a function definition is called a <a id="docstring_index_term"></a>docstring. Not only does it document the
...
<a href="#fun-def" id="ref-fun-def"><span><span>[1]</span></span></a>. Then we call the function and pass it two arguments, `monty` and `3`
<a href="#fun-call" id="ref-fun-call"><span><span>[2]</span></span></a>; these arguments fill the "placeholders" provided by the parameters and
...
Back in [4.1](#sec-back-to-the-basics) you saw that assignment works on values,
...
known as <a id="call_by_value_index_term"></a>call-by-value). In the following code, `set_up()` has two parameters,
...
Function definitions create a new, local <a id="scope_index_term"></a>scope for variables.
...
the so-called <a id="lgb_rule_index_term"></a>LGB rule of name resolution: local,
...
but look what happens when it is passed a list <a href="#list-arg" id="ref-list-arg"><span><span>[1]</span></span></a> — it fails to
...
a program helps you find logical errors, and is a kind of <a id="defensive_programming_index_term"></a>defensive programming.
...
Consider the `freq_words` function in [4.3](#code-freq-words1).
...
*[**Example 4.3 (code_freq_words1.py)**](https://www.nltk.org/book/pylisting/code_freq_words1.py): **Figure 4.3**: Poorly Designed Function to Compute Frequent Words*
...
In [4.4](#code-freq-words2) we <a id="refactor_index_term"></a>refactor this function,
and simplify its interface by dropping the `freqdist` parameter.

<a id="code-freq-words2"></a>
...
*[**Example 4.4 (code_freq_words2.py)**](https://www.nltk.org/book/pylisting/code_freq_words2.py): **Figure 4.4**: Well-Designed Function to Compute Frequent Words*
...
For the simplest functions, a one-line docstring is usually adequate (see [4.2](#code-get-text)).
...
Docstrings can include a <a id="doctest_block_index_term"></a>doctest block, illustrating the use of
...
clearly documented. [4.5](#code-sphinx) illustrates
...
*[**Example 4.5 (code_sphinx.py)**](https://www.nltk.org/book/pylisting/code_sphinx.py): **Figure 4.5**: Illustration of a complete docstring, consisting of a one-line summary,
...
<a id="sec-doing-more-with-functions"></a>
...
to other functions, so-called <a id="lambda_expressions_index_term"></a>lambda expressions. Supposing there
...
in function `search1()` in [4.6](#code-search-examples).

<a id="code-search-examples"></a>
...
*[**Example 4.6 (code_search_examples.py)**](https://www.nltk.org/book/pylisting/code_search_examples.py): **Figure 4.6**: Accumulating Output into a List*
...
function to generate all its output, we wrap it with a call to `list()` <a href="#listperm" id="ref-listperm"><span><span>[1]</span></span></a>.
...
> discussed below in [4.7](#sec-algorithm-design).
> The ability to generate permutations of a set of words is
> useful for creating data to test a grammar ([8.](https://www.nltk.org/book/ch08.html#chap-parse)).
{: .note }
...
`extract_property()` function we saw in [4.5](#sec-doing-more-with-functions).
...
These are called <a id="keyword_arguments_index_term"></a>keyword arguments.
...
<a id="sec-program-development"></a>
...
from the existing modules. This scenario is depicted in [4.7](#fig-multi-module).

<a id="fig-multi-module"></a>

![../images/multi-module.png](https://www.nltk.org/images/multi-module.png){: style="width: 360.0px; height: 237.60000000000002px;" }
...
The first time we call `find_words()` <a href="#find-words-1" id="ref-find-words-1"><span><span>[1]</span></span></a>, we get all three-letter
...
other two-letter word in our text. Now, the next time we call `find_words()` <a href="#find-words-3" id="ref-find-words-3"><span><span>[3]</span></span></a>
we use the same parameters as in [[1]](#find-words-1), but we get a different result!
...
the interpreter will print a <a id="stack_trace_index_term"></a>stack trace,
...
Python provides a <a id="debugger_index_term"></a>debugger which allows you to monitor the execution
of your program, specify line numbers where execution will stop (i.e. <a id="breakpoints_index_term"></a>breakpoints),
...
called. We'll start by calling the function without using the debugger <a href="#first-run" id="ref-first-run"><span><span>[1]</span></span></a>,
using the smallest possible input. The second time, we'll call it with the
debugger <a href="#second-run" id="ref-second-run"><span><span>[2]</span></span></a>.
...
This is called <a id="regression_testing_index_term"></a>regression testing, since it is meant to detect
...
The best known strategy is known as <a id="divide_and_conquer_index_term"></a>divide-and-conquer.
...
See [4.8](#fig-mergesort) for an illustration of this process.

<a id="fig-mergesort"></a>

![../images/mergesort.png](https://www.nltk.org/images/mergesort.png){: style="width: 650.1px; height: 345.0px;" }
...
For example, in order to detect duplicate entries in a list, we can <a id="pre_sort_index_term"></a>pre-sort
...
A common way to implement such methods uses <a id="recursion_index_term"></a>recursion.
...
the hierarchy in layers. The first layer is the synset itself <a href="#first-layer" id="ref-first-layer"><span><span>[1]</span></span></a>,
...
by finding the hyponyms of everything in the last layer <a href="#update-layer" id="ref-update-layer"><span><span>[3]</span></span></a>.
It also maintains a total of the number of synsets encountered so far <a href="#update-total" id="ref-update-total"><span><span>[2]</span></span></a>.
...
A <a id="letter_trie_index_term"></a>letter trie is a data structure that can be used
...
[4.9](#code-trie) demonstrates the recursive process of building a trie,
using Python dictionaries ([3](https://www.nltk.org/book/ch05.html#sec-dictionaries)).
...
*[**Example 4.9 (code_trie.py)**](https://www.nltk.org/book/pylisting/code_trie.py): **Figure 4.9**: Building a Letter Trie: A recursive function that builds a nested dictionary
...
data structure, such as an index. The listing in [4.10](#code-search-documents) implements a simple
text retrieval system for the Movie Reviews Corpus. By indexing the document collection it
provides much faster lookup.

<a id="code-search-documents"></a>
...
*[**Example 4.10 (code_search_documents.py)**](https://www.nltk.org/book/pylisting/code_search_documents.py): **Figure 4.10**: A Simple Text Retrieval System*
...
Any language models can now work with integers. See the listing in [4.11](#code-strings-to-ints)
for an example of how to do this for a tagged corpus.

<a id="code-strings-to-ints"></a>
...
*[**Example 4.11 (code_strings_to_ints.py)**](https://www.nltk.org/book/pylisting/code_strings_to_ints.py): **Figure 4.11**: Preprocess tagged corpus data, converting all words and tags to integers*
...
100,000 items using a list <a href="#vocab-list" id="ref-vocab-list"><span><span>[1]</span></span></a> or set <a href="#vocab-set" id="ref-vocab-set"><span><span>[2]</span></span></a>
of integers. The test statement will generate a random
item which has a 50% chance of being in the vocabulary <a href="#vocab-statement" id="ref-vocab-statement"><span><span>[3]</span></span></a>.
...
construct a meter of length 4: *V*4 = *{LL, SSL, SLS,
LSS, SSSS}*. Observe that we can split *V*4 into two
subsets, those starting with *L* and those starting with
*S*, as shown in [(1)](#ex-v4).
...
*[**Example 4.12 (code_virahanka.py)**](https://www.nltk.org/book/pylisting/code_virahanka.py): **Figure 4.12**: Four Ways to Compute Sanskrit Meter: (i) recursive; (ii) bottom-up dynamic programming;
...
`virahanka1()` to compute these meters, shown in [4.12](#code-virahanka).
Notice that, in order to compute *V*4 we first compute
*V*3 and *V*2. But to compute *V*3,
we need to first compute *V*2 and *V*1. This <a id="call_structure_index_term"></a>call
structure is depicted in [(2)](#ex-call-structure).

<a id="ex-call-structure"></a>

> (2)<br />
> ![tree_images/ch04-tree-1.png](https://www.nltk.org/book/tree_images/ch04-tree-1.png)

As you can see, *V*2 is computed twice.
This might not seem like a significant problem, but
it turns out to be rather wasteful as *n* gets large:
to compute *V*20 using this recursive technique, we
would compute *V*2 4,181 times;
and for *V*40 we would compute *V*2 63,245,986 times!
A much better alternative is to store the value of *V*2 in a table
and look it up whenever we need it. The same goes for other values, such
as *V*3 and so on. Function `virahanka2()` implements a
...
<a id="bottom_up_index_term"></a>bottom-up approach to dynamic programming. Unfortunately it turns out
...
using the <a id="top_down_index_term"></a>top-down approach to dynamic programming, which is
illustrated in the function `virahanka3()` in [4.12](#code-virahanka).
...
We will encounter it again in [4](https://www.nltk.org/book/ch08.html#sec-parsing).
...
patterns. For example, in [3.7](https://www.nltk.org/book/ch03.html#code-modal-tabulate) we saw a table of numbers
showing the frequency of particular modal verbs in the Brown Corpus, classified
by genre. The program in [4.13](#code-modal-plot) presents the same information in graphical
format. The output is shown in [4.14](#fig-modal-genre) (a color figure in the graphical display).

<a id="code-modal-plot"></a>
...
*[**Example 4.13 (code_modal_plot.py)**](https://www.nltk.org/book/pylisting/code_modal_plot.py): **Figure 4.13**: Frequency of Modals in Different Sections of the Brown Corpus*

<a id="fig-modal-genre"></a>

![../images/modal_genre.png](https://www.nltk.org/images/modal_genre.png){: style="width: 613.0px; height: 463.0px;" }

***Figure 4.14**: Bar Chart Showing Frequency of Modals in Different Sections of Brown Corpus: this
visualization was produced by the program in [4.13](#code-modal-plot).*
...
which is a library for producing raster (pixel) images <a href="#agg-backend" id="ref-agg-backend"><span><span>[1]</span></span></a>.
...
using `pyplot.savefig()` <a href="#pyplot-savefig" id="ref-pyplot-savefig"><span><span>[2]</span></span></a>. We specify the filename
...
nodes and edges, known as <a id="graphs_index_term"></a>graphs. It is
...
introduced in [5](https://www.nltk.org/book/ch02.html#sec-wordnet)). The program in [4.15](#code-networkx)
initializes an empty graph <a href="#define-graph" id="ref-define-graph"><span><span>[3]</span></span></a> then traverses
the WordNet hypernym hierarchy adding edges to
the graph <a href="#add-edge" id="ref-add-edge"><span><span>[1]</span></span></a>.
Notice that the traversal is recursive <a href="#recursive-traversal" id="ref-recursive-traversal"><span><span>[2]</span></span></a>,
applying the programming technique discussed in
[4.7](#sec-algorithm-design). The resulting display is shown in [4.16](#fig-dog-graph).

<a id="code-networkx"></a><a id="add-edge"></a><a id="recursive-traversal"></a><a id="define-graph"></a>
...
*[**Example 4.15 (code_networkx.py)**](https://www.nltk.org/book/pylisting/code_networkx.py): **Figure 4.15**: Using the NetworkX and Matplotlib Libraries*

<a id="fig-dog-graph"></a>

![../images/dog-graph.png](https://www.nltk.org/images/dog-graph.png){: style="width: 810.4000000000001px; height: 402.40000000000003px;" }
...
by the program in [4.15](#code-networkx).*
...
For example, we can open a CSV file called `lexicon.csv` <a href="#open-csv" id="ref-open-csv"><span><span>[1]</span></span></a>
and iterate over its rows <a href="#iterate-csv" id="ref-iterate-csv"><span><span>[2]</span></span></a>:
...
including working with sound files, is <a href="https://www.nltk.org/book/bibliography.html#guzdial2005" id="id1">(Guzdial, 2005)</a>.
...
good starting points are <a href="https://www.nltk.org/book/bibliography.html#harel2004" id="id2">(Harel, 2004)</a>, <a href="https://www.nltk.org/book/bibliography.html#levitin2004" id="id3">(Levitin, 2004)</a>, <a href="https://www.nltk.org/book/bibliography.html#knuth2006trees" id="id4">(Knuth, 2006)</a>.
Useful guidance on the practice of software development is provided
in <a href="https://www.nltk.org/book/bibliography.html#hunt2000" id="id5">(Hunt & Thomas, 2000)</a> and <a href="https://www.nltk.org/book/bibliography.html#mcconnell2004" id="id6">(McConnell, 2004)</a>.

### 4.11 Exercises

1. ☼ Find out more about sequence objects using Python's help facility.
...
1. ☼ Identify three operations that can be performed on both tuples
...
1. ☼ Find out how to create a tuple consisting of a single item.
   There are at least two ways to do this.

1. ☼ Create a list `words = ['is', 'NLP', 'fun', '?']`. Use
...
1. ☼ Read about the built-in comparison function `cmp`, by
   typing `help(cmp)`. How does it differ in behavior from
   the comparison operators?

1. ☼ Does the method for creating a sliding window of n-grams
   behave correctly for the two limiting cases: n = 1, and n = `len(sent)`?

1. ☼ We pointed out that when empty strings and empty lists occur
...
1. ☼ Use the inequality operators to compare strings, e.g.
...
1. ☼ Write code that removes whitespace at the beginning and end of a
   string, and normalizes whitespace between words to be a single
   space character.

   1. do this task using `split()` and `join()`
   1. do this task using regular expression substitutions
...
1. ◑ Create a list of words and store it in a variable `sent1`.
   ...
   1. Now define `text1` to be a list of lists of strings (e.g. to
      ...
      Explain.
   1. Load Python's `deepcopy()` function (i.e. `from copy import deepcopy`),
      consult its documentation, and test that it makes a fresh copy of any
      object.

1. ◑ Initialize an *n*-by-*m* list of lists of empty strings using list
...
1. ◑ Write code to initialize a two-dimensional array of sets called
...
1. ◑ Write a function `novel10(text)` that prints any word that
   appeared in the last 10% of a text that had not been encountered earlier.

1. ◑ Write a program that takes a sentence expressed as a single string,
...
1. ◑ Read up on Gematria, a method for assigning numbers to words, and for
   ...
   1. Process a corpus (e.g. `nltk.corpus.state_union`) and for each document, count how
      many of its words have the number 666.

   1. Write a function `decode()` to process a text, randomly replacing words with
      their Gematria equivalents, in order to discover the "hidden meaning" of the text.

1. ◑ Write a function `shorten(text, n)` to process a text, omitting the n
   most frequently occurring words of the text. How readable is it?

1. ◑ Write code to print out an index for a lexicon, allowing someone
   to look up words according to their meanings (or pronunciations; whatever
   properties are contained in lexical entries).

1. ◑ Write a list comprehension that sorts a list of WordNet synsets for
...
1. ◑ Write a function that takes a list of words (containing duplicates) and
...
1. ◑ Write a function that takes a text and a vocabulary as its arguments
...
1. ◑ Import the `itemgetter()` function from the `operator` module in Python's
...
1. ◑ Write a recursive function `lookup(trie, key)` that looks up a key in a trie,
...
1. ◑ Read up on "keyword linkage" (chapter 5 of <a href="https://www.nltk.org/book/bibliography.html#scott2006" id="id7">(Scott & Tribble, 2006)</a>). Extract keywords from
...
1. ◑ The Catalan numbers arise in many applications of combinatorial mathematics,
   including the counting of parse trees ([6](https://www.nltk.org/book/ch08.html#sec-grammar-development)). The series
   can be defined as follows: C0 = 1, and
   Cn+1 = Σ0..n (CiCn-i).

   1. Write a recursive function to compute nth Catalan number Cn.
   1. Now write another function that does this computation using dynamic programming.
   1. Use the `timeit` module to compare the performance of these functions as n
      increases.

1. ★
   Reproduce some of the results of <a href="https://www.nltk.org/book/bibliography.html#zhao07" id="id8">(Zhao & Zobel, 2007)</a> concerning authorship identification.

1. ★ Study gender-specific lexical choice, and see if you can
   reproduce some of the results of `http://www.clintoneast.com/articles/words.php`

1. ★ Write a recursive function that pretty prints a trie in alphabetically
   sorted order, e.g.:
...
1. ★ With the help of the trie data structure, write a recursive
...
1. ★ Obtain some raw text, in the form of a single, long string.
...
1. ★ Develop a simple extractive summarization tool, that prints the
...
1. ★ Write a program to implement a brute-force algorithm for
...
> by [Steven Bird](https://www.nltk.org/book/http://stevenbird.net/), [Ewan Klein](https://www.nltk.org/book/http://homepages.inf.ed.ac.uk/ewan/) and [Edward Loper](https://www.nltk.org/book/http://ed.loper.org/),
...
> \[[http://creativecommons.org/licenses/by-nc-nd/3.0/us/](https://www.nltk.org/book/http://creativecommons.org/licenses/by-nc-nd/3.0/us/)\].
>
