...
large bodies of linguistic data, or <a id="corpora_index_term"></a>corpora.
The goal of this chapter is to answer the following questions:

1. What are some useful text corpora and lexical resources, and how can we access them with Python?
1. Which Python constructs are most helpful for this work?
1. How do we avoid repeating ourselves when writing Python code?
...
<a id="sec-extracting-text-from-corpora"></a>
...
[1.](https://www.nltk.org/book/ch01.html#chap-introduction), such as the speeches known as the US Presidential
...
[1.](https://www.nltk.org/book/ch01.html#chap-introduction) also used various pre-defined texts that
...
> In [1](https://www.nltk.org/book/ch01.html#sec-computing-with-language-texts-and-words), we showed how you
...
> using one of the nine texts obtained as a result of doing `from nltk.book import *`. Now that you have started examining data from
...
> tasks from [1](https://www.nltk.org/book/ch01.html#sec-computing-with-language-texts-and-words):
>
> ```python
...
The previous example also showed how we can access the "raw" text of the book <a href="#raw-access" id="ref-raw-access"><span><span>[1]</span></span></a>,
...
[1.1](#tab-brown-sources) gives an example of each genre
(for a complete list, see `http://icame.uib.no/brown/bcm-los.html`).

<a id="tab-brown-sources"></a>
...
genres, a kind of linguistic inquiry known as <a id="stylistics_index_term"></a>stylistics.
...
presented systematically in [2](#sec-conditional-frequency-distributions),
...
might distinguish genres will be taken up again in [chap-data-intensive](https://www.nltk.org/book/ch06.html#chap-data-intensive).
...
In [1](https://www.nltk.org/book/ch01.html#sec-computing-with-language-texts-and-words), we looked at
the Inaugural Address Corpus,
but treated it as a single text. The graph in [fig-inaugural](https://www.nltk.org/book/ch01.html#fig-inaugural)
...
to lowercase using `w.lower()` <a href="#lowercase-startswith" id="ref-lowercase-startswith"><span><span>[1]</span></span></a>,
then checks if they start with either of the "targets"
`america` or `citizen` using `startswith()` [[1]](#lowercase-startswith).
Thus it will count words like *American's* and *Citizens*.
We'll learn about conditional frequency distributions in
[2](#sec-conditional-frequency-distributions); for now just consider
the output, shown in [1.1](#fig-inaugural2).
...
<a id="fig-inaugural2"></a>

![../images/inaugural2.png](https://www.nltk.org/images/inaugural2.png){: style="width: 646.74px; height: 292.32px;" }
...
[1.2](#tab-corpora) lists some of the corpora. For information about
...
<a id="fig-word-len-dist"></a>

![../images/word-len-dist.png](https://www.nltk.org/images/word-len-dist.png){: style="width: 613.0px; height: 463.0px;" }
...
established writing system, or are endangered. (See [7](#sec-further-reading-corpora)
for suggestions on how to locate language resources.)

### 1.8 Text Corpus Structure

We have seen a variety of corpus structures so far; these are
summarized in [1.3](#fig-text-corpus-structure).
...
<a id="fig-text-corpus-structure"></a>

![../images/text-corpus-structure.png](https://www.nltk.org/images/text-corpus-structure.png){: style="width: 607.1999999999999px; height: 129.6px;" }
...
be used to work with new corpora. [1.3](#tab-corpus) lists functionality
...
`corpus_root` <a href="#corpus-root-dict" id="ref-corpus-root-dict"><span><span>[1]</span></span></a>.
The second parameter of the `PlaintextCorpusReader` initializer <a href="#corpus-reader" id="ref-corpus-reader"><span><span>[2]</span></span></a>
can be a list of fileids, like `['a.txt', 'test/b.txt']`,
or a pattern that matches all fileids, like `'[abc]/.*\.txt'`
(see [3.4](https://www.nltk.org/book/ch03.html#sec-regular-expressions-word-patterns) for information
...
Journal component of the corpus <a href="#corpus-root-treebank" id="ref-corpus-root-treebank"><span><span>[1]</span></span></a>, and give a `file_pattern`
that matches the files contained within its subfolders <a href="#file-pattern" id="ref-file-pattern"><span><span>[2]</span></span></a> (using forward slashes).
...
<a id="sec-conditional-frequency-distributions"></a>

## 2 Conditional Frequency Distributions

We introduced frequency distributions in [3](https://www.nltk.org/book/ch01.html#sec-computing-with-language-simple-statistics).
...
type. A <a id="conditional_frequency_distribution_index_term"></a>conditional frequency distribution is a collection of
frequency distributions, each one for a different "condition". The
condition will often be the category of the text. [2.1](#fig-tally2)
...
![../images/tally2.png](https://www.nltk.org/images/tally2.png){: style="width: 412.29999999999995px; height: 130.2px;" }
...
So instead of processing a sequence of words <a href="#seq-words" id="ref-seq-words"><span><span>[1]</span></span></a>,
we have to process a sequence of pairs <a href="#seq-pairs" id="ref-seq-pairs"><span><span>[2]</span></span></a>:
...
In [1](#sec-extracting-text-from-corpora) we saw a conditional
...
For each genre <a href="#each-genre" id="ref-each-genre"><span><span>[2]</span></span></a>, we loop over every word in the genre <a href="#each-word" id="ref-each-word"><span><span>[3]</span></span></a>,
producing pairs consisting of the genre and the word <a href="#genre-word-pairs" id="ref-genre-word-pairs"><span><span>[1]</span></span></a>:
...
variable to inspect it <a href="#inspect-cfd" id="ref-inspect-cfd"><span><span>[1]</span></span></a>, and verify it has two conditions <a href="#conditions-cfd" id="ref-conditions-cfd"><span><span>[2]</span></span></a>:
...
The plot in [1.1](#fig-inaugural2) was based on a conditional frequency distribution
reproduced in the code below.
The condition is either of the words *america* or *citizen* <a href="#america-citizen" id="ref-america-citizen"><span><span>[2]</span></span></a>,
...
contains the year as the first four characters <a href="#first-four-chars" id="ref-first-four-chars"><span><span>[1]</span></span></a>.
...
The plot in [1.2](#fig-word-len-dist) was also based on a conditional frequency distribution,
reproduced below. This time, the condition is the name of the language
and the counts being plotted are derived from word lengths <a href="#lang-len-word" id="ref-lang-len-word"><span><span>[1]</span></span></a>.
...
(See the discussion of "generator expressions" in [4.2](https://www.nltk.org/book/ch04.html#sec-sequences)
for more about this.)
...
[3](https://www.nltk.org/book/ch01.html#sec-computing-with-language-simple-statistics).)
...
In [2.2](#code-random-text), we treat each word as a condition, and for each one
...
<a id="code-random-text"></a><a id="bigram-condition"></a>
...
*[**Example 2.2 (code_random_text.py)**](https://www.nltk.org/book/pylisting/code_random_text.py): **Figure 2.2**: Generating Random Text: this program obtains all bigrams
...
Their commonly-used methods are summarized in [2.1](#tab-conditionalfreqdist).

<a id="tab-conditionalfreqdist"></a>
...
> than using the interpreter. You can see this already in [2.2](#code-random-text) above.
...
> Remember that all code samples like [2.2](#code-random-text) are downloadable
> from `http://nltk.org/`.
{: .note }
...
efficient and reliable to localize this work inside a <a id="function_index_term"></a>function.
A function is just a named block of code that performs some well-defined
task, as we saw in [1](https://www.nltk.org/book/ch01.html#sec-computing-with-language-texts-and-words).
A function is usually defined to take some inputs, using special variables known as <a id="parameters_index_term"></a>parameters,
and it may produce a result, also known as a <a id="return_value_index_term"></a>return value.
...
These are <a id="local_variables_index_term"></a>local variables and are not accessible outside the function.
...
function to work out English plurals. The function `plural()` in [3.1](#code-plural)
takes a singular noun and generates a plural form, though it is not always
correct. (We'll discuss functions at greater length in [4.4](https://www.nltk.org/book/ch04.html#sec-functions).)
...
*[**Example 3.1 (code_plural.py)**](https://www.nltk.org/book/pylisting/code_plural.py): **Figure 3.1**: A Python Function: this function tries to work out the
...
(e.g., `word` in [3.1](#code-plural)). To call such functions, we give
the name of the object, a period, and then the name of the function.
These functions are usually known as <a id="methods_index_term"></a>methods.
...
<a id="module_index_term"></a>module. A collection of related modules is called a <a id="package_index_term"></a>package.
...
called a <a id="library_index_term"></a>library.
...
like the one we saw in [1](https://www.nltk.org/book/ch01.html#sec-computing-with-language-texts-and-words)
gives us information about word usage that might help in the preparation of
a dictionary. Standard terminology for lexicons is illustrated in [4.1](#fig-lexicon).
A <a id="lexical_entry_index_term"></a>lexical entry consists of a <a id="headword_index_term"></a>headword (also known as a <a id="lemma_index_term"></a>lemma)
along with additional information such as the part of speech and the sense
definition. Two distinct words having the same spelling are called <a id="homonyms_index_term"></a>homonyms.

<a id="fig-lexicon"></a>

![../images/lexicon.png](https://www.nltk.org/images/lexicon.png){: style="width: 504.0px; height: 223.20000000000002px;" }
...
words in a text corpus, as shown in [4.2](#code-unusual).
...
*[**Example 4.2 (code_unusual.py)**](https://www.nltk.org/book/pylisting/code_unusual.py): **Figure 4.2**: Filtering a Text: this program computes the vocabulary of a text,
then removes all items that occur in an existing wordlist,
leaving just the uncommon or mis-spelt words.*

There is also a corpus of <a id="stopwords_index_term"></a>stopwords, that is, high-frequency
...
![../images/target.png](https://www.nltk.org/images/target.png){: style="width: 652.5px; height: 133.79999999999998px;" }
...
A wordlist is useful for solving word puzzles, such as the one in [4.3](#fig-target).
Our program iterates through every word and, for each one, checks whether it meets
the conditions. It is easy to check obligatory letter <a href="#obligatory-letter" id="ref-obligatory-letter"><span><span>[2]</span></span></a>
and length constraints <a href="#length-constraint" id="ref-length-constraint"><span><span>[1]</span></span></a> (and we'll
...
The `FreqDist` comparison method <a href="#freqdist-compare" id="ref-freqdist-compare"><span><span>[3]</span></span></a> permits us to check that
...
We can see this and some other patterns in the graph in [4.4](#fig-cfd-gender),
...
<a id="fig-cfd-gender"></a>

![../images/cfd-gender.png](https://www.nltk.org/images/cfd-gender.png){: style="width: 613.0px; height: 463.0px;" }
...
`entry` with *two* variable names, `word, pron` <a href="#word-pron" id="ref-word-pron"><span><span>[1]</span></span></a>.
...
three phones <a href="#len-pron-three" id="ref-len-pron-three"><span><span>[2]</span></span></a>. If the condition is true, it assigns the contents
of `pron` to three new variables `ph1`, `ph2` and `ph3`. Notice the unusual
form of the statement which does that work <a href="#tuple-assignment" id="ref-tuple-assignment"><span><span>[3]</span></span></a>.
...
sets of words. Here we find all the *p*-words consisting of three sounds <a href="#p3-words" id="ref-p3-words"><span><span>[2]</span></span></a>,
and group them according to their first and last sounds <a href="#group-first-last" id="ref-group-first-last"><span><span>[1]</span></span></a>.
...
structure, which we will study systematically in [3](https://www.nltk.org/book/ch05.html#sec-dictionaries).
We look up a dictionary by giving its name followed by a <a id="key_index_term"></a>key
(such as the word `'fire'`) inside square brackets <a href="#dict-key" id="ref-dict-key"><span><span>[1]</span></span></a>.
...
If we try to look up a non-existent key <a href="#dict-key-error" id="ref-dict-key-error"><span><span>[2]</span></span></a>, we get a `KeyError`.
...
so we tweak our version by assigning a value for this key <a href="#dict-assign" id="ref-dict-assign"><span><span>[3]</span></span></a>
...
Another example of a tabular lexicon is the <a id="comparative_wordlist_index_term"></a>comparative wordlist.
...
we will return to this topic in [11.](https://www.nltk.org/book/ch11.html#chap-data).
...
<a id="wordnet_index_term"></a>WordNet is a semantically-oriented dictionary of English,
...
Consider the sentence in [(1a)](#ex-car1).
If we replace the word *motorcar* in [(1a)](#ex-car1) by *automobile*,
to get [(1b)](#ex-car2), the meaning of the sentence stays pretty much the same:

<a id="ex-car1"></a><a id="ex-car2"></a>
```
...
We can get all the lemmas for a given synset <a href="#get-lemmas" id="ref-get-lemmas"><span><span>[1]</span></span></a>,
look up a particular lemma <a href="#lookup-lemma" id="ref-lookup-lemma"><span><span>[2]</span></span></a>,
get the synset corresponding to a lemma <a href="#get-synset" id="ref-get-synset"><span><span>[3]</span></span></a>,
and get the "name" of a lemma <a href="#get-name" id="ref-get-name"><span><span>[4]</span></span></a>:
...
<a id="unique_beginners_index_term"></a>unique beginners or root synsets. Others, such as *gas guzzler* and
*hatchback*, are much more specific. A small portion of a concept
hierarchy is illustrated in [5.1](#fig-wn-hierarchy).

<a id="fig-wn-hierarchy"></a>

![../images/wordnet-hierarchy.png](https://www.nltk.org/images/wordnet-hierarchy.png){: style="width: 451.25px; height: 245.0px;" }
...
Hypernyms and hyponyms are called <a id="lexical_relations_index_term"></a>lexical relations because they relate one
...
components (<a id="meronyms_index_term"></a>meronyms) or to the things they are contained in (<a id="holonyms_index_term"></a>holonyms).
...
<a id="sec-further-reading-corpora"></a>
...
This chapter has touched on the field of <a id="corpus_linguistics_index_term"></a>Corpus Linguistics. Other useful books in this
area include <a href="https://www.nltk.org/book/bibliography.html#biber1998" id="id1">(Biber, Conrad, & Reppen, 1998)</a>, <a href="https://www.nltk.org/book/bibliography.html#mcenery2006" id="id2">(McEnery, 2006)</a>, <a href="https://www.nltk.org/book/bibliography.html#meyer2002" id="id3">(Meyer, 2002)</a>, <a href="https://www.nltk.org/book/bibliography.html#sampson2005" id="id4">(Sampson & McCarthy, 2005)</a>, <a href="https://www.nltk.org/book/bibliography.html#scott2006" id="id5">(Scott & Tribble, 2006)</a>.
Further readings in quantitative data analysis in linguistics are:
<a href="https://www.nltk.org/book/bibliography.html#baayen2008" id="id6">(Baayen, 2008)</a>, <a href="https://www.nltk.org/book/bibliography.html#gries2009" id="id7">(Gries, 2009)</a>, <a href="https://www.nltk.org/book/bibliography.html#woods1986" id="id8">(Woods, Fletcher, & Hughes, 1986)</a>.

The original description of WordNet is <a href="https://www.nltk.org/book/bibliography.html#fellbaum1998" id="id9">(Fellbaum, 1998)</a>.
...
For a study of WordNet similarity measures, see <a href="https://www.nltk.org/book/bibliography.html#budanitsky2006ewb" id="id10">(Budanitsky & Hirst, 2006)</a>.

Other topics touched on in this chapter were phonetics and lexical semantics,
and we refer readers to chapters 7 and 20 of <a href="https://www.nltk.org/book/bibliography.html#jurafskymartin2008" id="id11">(Jurafsky & Martin, 2008)</a>.
...
1. ☼ Use the Brown corpus reader `nltk.corpus.brown.words()` or the Web text corpus
   reader `nltk.corpus.webtext.words()` to access some sample text in two different genres.
1. ☼ Read in the texts of the *State of the Union* addresses, using the
...
1. ☼ In the discussion of comparative wordlists, we created an object
...
1. ☼ According to Strunk and White's *Elements of Style*,
...
1. ◑ Define a conditional frequency distribution over the Names corpus
   that allows you to see which *initial* letters are more frequent for males
   vs. females (cf. [4.4](#fig-cfd-gender)).
1. ◑ Pick a pair of texts and study the differences between them,
...
1. ◑ Read the BBC News article: *UK's Vicky Pollards 'left behind'* `http://news.bbc.co.uk/1/hi/education/6173441.stm`.
...
1. ◑ Investigate the table of modal distributions and look for other patterns.
...
1. ◑ The CMU Pronouncing Dictionary contains multiple pronunciations
...
1. ◑ What percentage of noun synsets have no hyponyms?
   You can get all noun synsets using `wn.all_synsets('n')`.
1. ◑ Define a function `supergloss(s)` that takes a synset `s` as its argument
...
1. ◑ Write a program to generate a table of lexical diversity scores (i.e. token/type ratios), as we saw in
   [1.1](https://www.nltk.org/book/ch01.html#tab-brown-types). Include the full set of Brown Corpus genres (`nltk.corpus.brown.categories()`).
   Which genre has the lowest diversity (greatest number of tokens per type)?
   Is this what you would have expected?
1. ◑ Write a function that finds the 50 most frequently occurring words
   of a text that are not stopwords.
1. ◑ Write a program to print the 50 most frequent bigrams
...
1. ◑ Write a program to create a table of word frequencies by genre,
   like the one given in [1](#sec-extracting-text-from-corpora) for modals.
   Choose your own words and try to find words whose presence
   (or absence) is typical of a genre. Discuss your findings.
1. ◑ Write a function `word_freq()` that takes a word and the name of a section
   of the Brown Corpus as arguments, and computes the frequency of the word
   in that section of the corpus.
1. ◑ Write a program to guess the number of syllables contained in a text,
   making use of the CMU Pronouncing Dictionary.
1. ◑ Define a function `hedge(text)` which processes a
...
1. ★ **Zipf's Law**:
   ...
   1. Generate random text, e.g., using `random.choice("abcdefg ")`,
...
1. ★ Modify the text generation program in [2.2](#code-random-text) further, to
   ...
   1. Select a particular genre, such as a section of the Brown Corpus,
   ...
   1. Now train your system using two distinct genres and experiment
      with generating text in the hybrid genre. Discuss your observations.
1. ★ Define a function `find_language()` that takes a string
...
1. ★ What is the branching factor of the noun hypernym hierarchy?
   I.e. for every noun synset that has hyponyms — or children in the
   hypernym hierarchy — how many do they have on average?
   You can get all noun synsets using `wn.all_synsets('n')`.
1. ★ The polysemy of a word is the number of senses it has.
...
1. ★ Use one of the predefined similarity measures to score
   ...
   by <a href="https://www.nltk.org/book/bibliography.html#millercharles1998" id="id12">(Miller & Charles, 1998)</a>:
