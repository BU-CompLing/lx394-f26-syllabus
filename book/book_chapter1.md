...
1. What tools and techniques does the Python programming language provide for such work?
1. What are some of the interesting challenges of natural language processing?
...
<a id="sec-computing-with-language-texts-and-words"></a>
...
to type directly into the interactive <a id="interpreter_index_term"></a>interpreter —
...
This produced a <a id="syntax_error_index_term"></a>syntax error. In Python, it doesn't make sense
...
<a id="fig-nltk-downloader"></a>

![../images/nltk-downloader.png](https://www.nltk.org/images/nltk-downloader.png){: style="width: 668.0px; height: 276.0px;" }
...
This positional information can be displayed using a <a id="dispersion_plot_index_term"></a>dispersion plot.
...
<a id="fig-inaugural"></a>

![../images/inaugural.png](https://www.nltk.org/images/inaugural.png){: style="width: 738.0px; height: 324.0px;" }
...
A <a id="token_index_term"></a>token is the technical name for a sequence of characters
...
<a id="sorted-set"></a><a id="len-set"></a>
...
<a href="#sorted-set" id="ref-sorted-set"><span><span>[1]</span></span></a>, we obtain a sorted list of vocabulary items, beginning
...
obtain this number <a href="#len-set" id="ref-len-set"><span><span>[2]</span></span></a>. Although it has 44,764 tokens, this book
has only 2,789 distinct words, or "word types."
A <a id="word_type_index_term"></a>word type is the form or spelling of the word independently of its
...
unique items <a id="types_index_term"></a>types instead of word types.
...
task for us is called a <a id="function_index_term"></a>function, and
...
<a id="fun-parameter1"></a><a id="locvar"></a><a id="fun-parameter2"></a>
...
> that Python expects an <a id="indented_code_block_index_term"></a>indented code block to appear next.
> It is up to you to do the indentation, by typing four
> spaces or hitting the tab key. To finish the indented block just
> enter a blank line.
{: .warning }

In the definition of `lexical_diversity()` <a href="#fun-parameter1" id="ref-fun-parameter1"><span><span>[1]</span></span></a>, we
specify a <a id="parameter_index_term"></a>parameter named `text` . This parameter is
...
function is used <a href="#locvar" id="ref-locvar"><span><span>[2]</span></span></a>. Similarly, `percentage()` is defined to
take two parameters, named `count` and `total` <a href="#fun-parameter2" id="ref-fun-parameter2"><span><span>[3]</span></span></a>.
...
To recap, we use or <a id="call_index_term"></a>call a function such as `lexical_diversity()` by typing its name, followed
...
function is an <a id="argument_index_term"></a>argument to the function.
...
<a id="sec-a-closer-look-at-python-texts-as-lists-of-words"></a>
...
is known as a <a id="list_index_term"></a>list in Python: it is how we store a text.
We can inspect it by typing the name <a href="#inspect-var" id="ref-inspect-var"><span><span>[1]</span></span></a>. We can ask for its length <a href="#len-sent" id="ref-len-sent"><span><span>[2]</span></span></a>.
We can even apply our own `lexical_diversity()` function to it <a href="#apply-function" id="ref-apply-function"><span><span>[3]</span></span></a>.

<a id="inspect-var"></a><a id="len-sent"></a><a id="apply-function"></a>
...
Adding two lists <a href="#list-plus-list" id="ref-list-plus-list"><span><span>[1]</span></span></a> creates a new list
with everything from the first list, followed
by everything from the second list:

<a id="list-plus-list"></a>
...
> This special use of the addition operation is called <a id="concatenation_index_term"></a>concatenation;
...
What if we want to add a single item to a list? This is known as <a id="appending_index_term"></a>appending.
...
represents this position is the item's <a id="index_index_term"></a>index. We instruct Python
...
known as <a id="slicing_index_term"></a>slicing.
...
Instead, it is a <a id="runtime_error_index_term"></a>runtime error, and it produces a `Traceback` message that
...
list <a href="#slice2" id="ref-slice2"><span><span>[1]</span></span></a>, and we can omit the second number if the slice goes to the end <a href="#slice3" id="ref-slice3"><span><span>[2]</span></span></a>:

<a id="slice2"></a><a id="slice3"></a>
...
In the next example, we put `sent[0]` on the left of the equals sign <a href="#list-assignment" id="ref-list-assignment"><span><span>[1]</span></span></a>. We can also
replace an entire slice with new material <a href="#slice-assignment" id="ref-slice-assignment"><span><span>[2]</span></span></a>. A consequence of this
last change is that the list only has four elements, and accessing a later value
generates an error <a href="#list-error" id="ref-list-error"><span><span>[3]</span></span></a>.

<a id="list-assignment"></a><a id="slice-assignment"></a><a id="list-error"></a>
...
defining a <a id="variable_index_term"></a>variable `sent1`, as follows:
...
called <a id="assignment_index_term"></a>assignment. It does not generate any output;
...
> Take care with your choice of names (or <a id="identifiers_index_term"></a>identifiers) for Python
...
or <a id="strings_index_term"></a>strings. For example, we can assign a string to a variable <a href="#assign-string" id="ref-assign-string"><span><span>[1]</span></span></a>,
index a string <a href="#index-string" id="ref-index-string"><span><span>[2]</span></span></a>, and slice a string <a href="#slice-string" id="ref-slice-string"><span><span>[3]</span></span></a>:

<a id="assign-string"></a><a id="index-string"></a><a id="slice-string"></a>
...
<a id="sec-computing-with-language-simple-statistics"></a>
...
![../images/tally.png](https://www.nltk.org/images/tally.png){: style="width: 231.8px; height: 155.0px;" }

***Figure 3.1**: Counting Words Appearing in a Text (a frequency distribution)*

The table in [3.1](#fig-tally) is known as a <a id="frequency_distribution_index_term"></a>frequency distribution,
...
<a id="freq-dist-call"></a><a id="freq-dist-inspect"></a><a id="freq-dist-most-common"></a>
...
argument <a href="#freq-dist-call" id="ref-freq-dist-call"><span><span>[1]</span></span></a>. We can inspect the total number of words ("outcomes")
that have been counted up <a href="#freq-dist-inspect" id="ref-freq-dist-inspect"><span><span>[2]</span></span></a> — 260,819 in the
case of *Moby Dick*. The expression `most_common(50)` gives us a list of
the 50 most frequently occurring types in the text <a href="#freq-dist-most-common" id="ref-freq-dist-most-common"><span><span>[3]</span></span></a>.
...
<a id="fig-fdist-moby"></a>

![../images/fdist-moby.png](https://www.nltk.org/images/fdist-moby.png){: style="width: 207.20000000000002px; height: 116.2px;" }
...
only, the so-called <a id="hapaxes_index_term"></a>hapaxes? View them by typing `fdist1.hapaxes()`.
...
<a id="ex-set-comprehension"></a>
<a id="ex-set-comprehension-math"></a>
<a id="ex-set-comprehension-python"></a>
...
A <a id="collocation_index_term"></a>collocation is a sequence of words that occur together
...
a list of word pairs, also known as <a id="bigrams_index_term"></a>bigrams. This is easily
...
<a id="word-lengths"></a><a id="freq-word-lengths"></a><a id="freq-word-lengths-size"></a>
...
<a href="#word-lengths" id="ref-word-lengths"><span><span>[1]</span></span></a>,
and the `FreqDist` then counts the number of times each of these
occurs <a href="#freq-word-lengths" id="ref-freq-word-lengths"><span><span>[2]</span></span></a>. The result <a href="#freq-word-lengths-size" id="ref-freq-word-lengths-size"><span><span>[3]</span></span></a> is a distribution containing
...
is known as <a id="control_index_term"></a>control, and is the focus of this section.

### 4.1 Conditionals

Python supports a wide range of operators, such as `<` and `>=`, for
testing the relationship between values. The full set of these <a id="relational_operators_index_term"></a>relational
operators is shown in [4.1](#tab-inequalities).

<a id="tab-inequalities"></a>
...
<a id="conditional_expression_index_term"></a>conditional expression, or `if` statement, is satisfied. We
already saw examples of conditional tests in code like `[w for w in sent7 if len(w) < 4]`. In the following program, we have created a
...
When we use the Python interpreter we have to add an extra blank line <a href="#blank-line" id="ref-blank-line"><span><span>[1]</span></span></a>
...
An `if` statement is known as a <a id="control_structure_index_term"></a>control structure
...
<a id="sec-automatic-natural-language-understanding"></a>
...
In <a id="word_sense_disambiguation_index_term"></a>word sense disambiguation we want to work out
...
<a id="ex-lost-children"></a>

```
(3) a. The lost children were found by the _searchers_ (agentive)
    b. The lost children were found by the _mountain_ (locative)
    c. The lost children were found by the _afternoon_ (temporal) 
```
...
Answering this question involves finding the <a id="antecedent_index_term"></a>antecedent of the pronoun *they*,
either thieves or paintings. Computational techniques for tackling this problem
include <a id="anaphora_resolution_index_term"></a>anaphora resolution — identifying what a pronoun or noun phrase
refers to — and <a id="semantic_role_labeling_index_term"></a>semantic role labeling — identifying how a noun phrase
...
<a id="question_answering_index_term"></a>question answering and <a id="machine_translation_index_term"></a>machine translation. In the first case,
a machine should be able to answer a user's questions relating to collection of texts:

<a id="ex-qa-application"></a>
...
a process called <a id="text_alignment_index_term"></a>text alignment. Once we have a million or more sentence
...
has been a linguistic one, namely the <a id="turing_test_index_term"></a>Turing Test: can a dialogue system,
...
![../images/dialogue.png](https://www.nltk.org/images/dialogue.png){: style="width: 600.0px; height: 324.0px;" }
...
"pipeline" of some language understanding <a id="components_index_term"></a>components.
...
```
(7) a. Text: David Golinkin is the editor or author of eighteen books, and over 150 responsa, articles, sermons and books
...
- Indurkhya, Nitin and Fred Damerau (eds, 2010) *Handbook of Natural Language Processing*
  (Second Edition) Chapman & Hall/CRC. 2010. <a href="https://www.nltk.org/book/bibliography.html#indurkhyadamerau2010" id="id1">(Indurkhya & Damerau, 2010)</a> <a href="https://www.nltk.org/book/bibliography.html#dale00handbook" id="id2">(Dale, Moisl, & Somers, 2000)</a>
- Jurafsky, Daniel and James Martin (2008) *Speech and Language Processing* (Second Edition). Prentice Hall.
  <a href="https://www.nltk.org/book/bibliography.html#jurafskymartin2008" id="id3">(Jurafsky & Martin, 2008)</a>
- Mitkov, Ruslan (ed, 2003) *The Oxford Handbook of Computational Linguistics*. Oxford University Press.
  (second edition expected in 2010). <a href="https://www.nltk.org/book/bibliography.html#mitkov02handbook" id="id4">(Mitkov, 2002)</a>
...
<a href="#id8" id="id5"><span class="problematic">[Finegan2007]\_</span></a>, <a href="https://www.nltk.org/book/bibliography.html#ogrady2004" id="id6">(O'Grady et al, 2004)</a>, <a href="https://www.nltk.org/book/bibliography.html#osu2007" id="id7">(OSU, 2007)</a>. You might like to consult
...
1. ☼ Consider the following Python expression: `len(set(text4))`.
   State the purpose of this expression. Describe the two steps
   involved in performing this computation.

1. ☼ Review [2](#sec-a-closer-look-at-python-texts-as-lists-of-words)
...
1. ☼ Consider the following two expressions, which have the same
   value. Which one will typically be more relevant in NLP? Why?

   1. `"Monty Python"[6:12]`
   1. `["Monty", "Python"][1]`

1. ☼ We have seen how to represent a sentence as a list of words, where
   each word is a sequence of characters. What does `sent1[2][2]` do?
   Why? Experiment with other index values.

1. ☼ The first sentence of `text3` is provided to you in the
...
1. ☼ Review the discussion of conditionals in [4](#sec-making-decisions).
   Find all words in the Chat Corpus (`text5`)
   starting with the letter *b*. Show them in alphabetical order.

1. ☼ Type the expression `list(range(10))` at the interpreter prompt.
...
1. ◑ Use `text9.index()` to find the index of the word *sunset*.
...
1. ◑ Using list addition, and the `set` and `sorted` operations, compute the
...
1. ◑ Write the slice expression that extracts the last two words of `text2`.

1. ◑ Find all the four-letter words in the Chat Corpus (`text5`).
   With the help of a frequency distribution (`FreqDist`), show these
   words in decreasing order of frequency.

1. ◑ Review the discussion of looping with conditions in [4](#sec-making-decisions).
...
1. ◑ Write expressions for finding all words in `text6` that
...
1. ◑ Define `sent` to be the list of words
...
1. ◑ What does the following Python code do? `sum(len(w) for w in text1)`
   Can you use it to work out the average word length of a text?

1. ◑ Define a function called `vocab_size(text)` that has a single
   parameter for the text, and which returns the vocabulary size of the text.

1. ◑ Define a function `percent(word, text)` that calculates
   how often a given word occurs in a text, and expresses the result
   as a percentage.

1. ◑ We have been using sets to store vocabularies. Try the following
