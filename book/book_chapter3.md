...
1. How can we write programs to access text from local files and
   from the web, in order to get hold of an unlimited range of
   language material?
1. How can we split documents up into individual words and
   punctuation symbols, so we can carry out the same kinds of
   analysis we did with text corpora in earlier chapters?
1. How can we write programs to produce formatted output
...
words and punctuation, as we saw in [1.](https://www.nltk.org/book/ch01.html#chap-introduction). This step is
...
the right index values to use for slicing the string <a href="#raw-slice" id="ref-raw-slice"><span><span>[1]</span></span></a>.
...
Recall that the `'\n'` characters are <a id="newlines_index_term"></a>newlines; this
...
[3.1](#fig-pipeline1) summarizes what we have covered in this section, including the process
of building a vocabulary that we saw in [1.](https://www.nltk.org/book/ch01.html#chap-introduction). (One step, normalization,
will be discussed in [3.6](#sec-normalizing-text).)

<a id="fig-pipeline1"></a>

![../images/pipeline1.png](https://www.nltk.org/images/pipeline1.png){: style="width: 571.5px; height: 212.7px;" }
...
data type known as a <a id="string_index_term"></a>string. In this section we explore strings
in detail, and show the connection between strings, words, texts and files.

### Basic Operations with Strings

Strings are specified using single quotes <a href="#single-quotes" id="ref-single-quotes"><span><span>[1]</span></span></a>
or double quotes <a href="#double-quotes" id="ref-double-quotes"><span><span>[2]</span></span></a>, as shown below.
If a string contains a single quote, we must backslash-escape
the quote <a href="#backslash-escape" id="ref-backslash-escape"><span><span>[3]</span></span></a> so Python knows a literal quote character is intended,
or else put the string in double quotes [[2]](#double-quotes).
Otherwise, the quote inside the string <a href="#unescaped-quote" id="ref-unescaped-quote"><span><span>[4]</span></span></a>
...
We need to use backslash <a href="#string-backslash" id="ref-string-backslash"><span><span>[1]</span></span></a> or parentheses <a href="#string-parentheses" id="ref-string-parentheses"><span><span>[2]</span></span></a> so that
...
First let's look at the `+` operation, known as <a id="concatenation_index_term"></a>concatenation <a href="#string-concatenation" id="ref-string-concatenation"><span><span>[1]</span></span></a>.
...
the words. We can even multiply strings <a href="#string-multiplication" id="ref-string-multiplication"><span><span>[2]</span></span></a>:
...
As we saw in [2](https://www.nltk.org/book/ch01.html#sec-a-closer-look-at-python-texts-as-lists-of-words) for lists, strings are indexed, starting from zero.
...
where `-1` is the index of the last character <a href="#last-character" id="ref-last-character"><span><span>[1]</span></span></a>.
...
<a id="fig-string-slicing"></a>

![../images/string-slicing.png](https://www.nltk.org/images/string-slicing.png){: style="width: 388.75px; height: 129.25px;" }
...
we used for lists (see [3.2](#fig-string-slicing)).
...
we haven't seen yet, is shown in [3.2](#tab-string-methods). For more information on strings, type
`help(str)` at the Python prompt.

<a id="tab-string-methods"></a>
...
shown in [4.2](https://www.nltk.org/book/ch01.html#tab-word-tests); all methods produce a new string or list*

### The Difference between Lists and Strings

Strings and lists are both kinds of <a id="sequence_index_term"></a>sequence. We can pull them
...
code is tokenize a string into a list of strings ([3.7](#sec-tokenization)).
Conversely, when we want to write our results to a file, or to a terminal,
we will usually format them as a string ([3.9](#sec-formatting)).
...
This is because strings are <a id="immutable_index_term"></a>immutable — you can't change a
string once you have created it. However, lists are <a id="mutable_index_term"></a>mutable,
...
character is assigned a number, called a <a id="code_point_index_term"></a>code point. In Python, code
...
Unicode is called <a id="decoding_index_term"></a>decoding. Conversely, to write out Unicode to a
file or a terminal, we first need to translate it into a suitable
encoding — this translation out of Unicode is called <a id="encoding_index_term"></a>encoding,
and is illustrated in [3.3](#fig-unicode).

<a id="fig-unicode"></a>

![../images/unicode.png](https://www.nltk.org/images/unicode.png){: style="width: 466.70000000000005px; height: 234.20000000000002px;" }

***Figure 3.3**: Unicode Decoding and Encoding*

From a Unicode perspective, characters are abstract entities which can
be realized as one or more <a id="glyphs_index_term"></a>glyphs. Only glyphs can appear on a
...
If you replace
`c.encode('utf8')` in <a href="#unicode-info" id="ref-unicode-info"><span><span>[1]</span></span></a> with `c`, and if your system supports UTF-8,
...
string like `'latin-1'`, `'big5'` or `'utf-8'` (see [3.4](#fig-polish-utf8)).

<a id="fig-polish-utf8"></a>

![../images/polish-utf8.png](https://www.nltk.org/images/polish-utf8.png){: style="width: 605.0px; height: 326.0px;" }
...
<a id="sec-regular-expressions-word-patterns"></a>
...
in [4.2](https://www.nltk.org/book/ch01.html#tab-word-tests).
...
The `.` <a id="wildcard_index_term"></a>wildcard symbol matches any single character.
...
![../images/T9.png](https://www.nltk.org/images/T9.png){: style="width: 297.6px; height: 131.0px;" }

***Figure 3.5**: T9: Text on 9 Keys*

The <a id="t9_index_term"></a>T9 system is used for entering text on mobile phones (see [3.5](#fig-t9)). Two or more words that
are entered with the same sequence of keystrokes are known as <a id="textonyms_index_term"></a>textonyms.
...
Note that the `+` and `*` symbols are sometimes referred to as <a id="kleene_closures_index_term"></a>Kleene closures,
or simply <a id="closures_index_term"></a>closures.
...
The meta-characters we have seen are summarized in [3.3](#tab-regexp-meta-characters1).

<a id="tab-regexp-meta-characters1"></a>
...
it is a <a id="raw_string_index_term"></a>raw string. For example, the raw string `r'\band\b'`
...
<a id="sec-useful-applications-of-regular-expressions"></a>
...
`<.*>` <a href="#single-token-wildcard" id="ref-single-token-wildcard"><span><span>[1]</span></span></a> which will match any single token, and enclose it in parentheses so only the
...
<a href="#three-word-phrases" id="ref-three-word-phrases"><span><span>[2]</span></span></a>. The last example finds sequences of three or more words starting with
the letter *l* <a href="#letter-l" id="ref-letter-l"><span><span>[3]</span></span></a>.
...
> [11.](https://www.nltk.org/book/ch11.html#chap-data).
{: .note }
...
By using `lower()`, we have <a id="normalized_index_term"></a>normalized the text to lowercase so that
...
words (illustrated in [3.6](#code-stemmer-indexing), which uses *object oriented*
programming techniques that are outside the scope of this book, string formatting
techniques to be covered in [3.9](#sec-formatting), and the `enumerate()` function
to be explained in [4.2](https://www.nltk.org/book/ch04.html#sec-sequences)).

<a id="code-stemmer-indexing"></a>
...
*[**Example 3.6 (code_stemmer_indexing.py)**](https://www.nltk.org/book/pylisting/code_stemmer_indexing.py): **Figure 3.6**: Indexing a Text Using a Stemmer*
...
> Another normalization task involves identifying <a id="non_standard_words_index_term"></a>non-standard words
...
any space characters in the string <a href="#split-space" id="ref-split-space"><span><span>[1]</span></span></a> since this results
in tokens that contain a `\n` newline character; instead we need
to match any number of spaces, tabs, or newlines <a href="#split-whitespace" id="ref-split-whitespace"><span><span>[2]</span></span></a>:
...
[3.4](#tab-re-symbols) lists the regular expression character class symbols we have
seen in this section, in addition to some other useful symbols.

<a id="tab-re-symbols"></a>
...
Tokenization is an instance of a more general problem of <a id="segmentation_index_term"></a>segmentation.
...
this by including the Punkt sentence segmenter <a href="https://www.nltk.org/book/bibliography.html#kissstrunk2006" id="id1">(Kiss & Strunk, 2006)</a>.
...
For another approach to sentence segmentation, see [2](https://www.nltk.org/book/ch06.html#sec-further-examples-of-supervised-classification).
...
heavily for "chunking" in [7.](https://www.nltk.org/book/ch07.html#chap-chunk)).
...
The `segment()` function in [3.7](#code-segment) demonstrates that we can
...
*[**Example 3.7 (code_segment.py)**](https://www.nltk.org/book/pylisting/code_segment.py): **Figure 3.7**: Reconstruct Segmented Text from String Representation:
...
lexical items. Following <a href="https://www.nltk.org/book/bibliography.html#brent1995" id="id2">(Brent, 1995)</a>, we can define an <a id="objective_function_index_term"></a>objective function,
...
![../images/brent.png](https://www.nltk.org/images/brent.png){: style="width: 711.3px; height: 267.59999999999997px;" }
...
*[**Example 3.9 (code_evaluate.py)**](https://www.nltk.org/book/pylisting/code_evaluate.py): **Figure 3.9**: Computing the Cost of Storing the Lexicon and Reconstructing the Source Text*

The final step is to search for the pattern of zeros and ones that minimizes this objective
function, shown in [3.10](#code-anneal). Notice that the best segmentation includes "words" like
...
*[**Example 3.10 (code_anneal.py)**](https://www.nltk.org/book/pylisting/code_anneal.py): **Figure 3.10**: Non-Deterministic Search Using Simulated Annealing: begin searching
...
we want to <a id="export_index_term"></a>export our data to a particular file format for use
...
The curly brackets `'{}'` mark the presence of a <a id="replacement_field_index_term"></a>replacement
...
replacement fields is called a <a id="format_string_index_term"></a>format string.
...
padded to width 6. It is right-justified by default for numbers <a href="#right-justified" id="ref-right-justified"><span><span>[1]</span></span></a>,
but we can precede the width specifier with a `'<'` alignment option to make numbers left-justified <a href="#left-justified" id="ref-left-justified"><span><span>[2]</span></span></a>.
...
Recall that in [1](https://www.nltk.org/book/ch02.html#sec-extracting-text-from-corpora) we saw
...
of headings and column widths, as shown in [3.11](#code-modal-tabulate).
Note the clear separation between the language processing work,
and the tabulation of results.

<a id="code-modal-tabulate"></a>
...
*[**Example 3.11 (code_modal_tabulate.py)**](https://www.nltk.org/book/pylisting/code_modal_tabulate.py): **Figure 3.11**: Frequency of Modals in Different Sections of the Brown Corpus*

Recall from the listing in [3.6](#code-stemmer-indexing) that we used a format string
...
We have seen how to read text from files ([3.1](#sec-accessing-text)).
...
Chapters 2 and 3 of <a href="https://www.nltk.org/book/bibliography.html#jurafskymartin2008" id="id3">(Jurafsky & Martin, 2008)</a> contain more advanced
material on regular expressions and morphology. For more extensive
discussion of text processing with Python see <a href="https://www.nltk.org/book/bibliography.html#mertz2003tpp" id="id4">(Mertz, 2003)</a>.
For information about normalizing non-standard words see <a href="https://www.nltk.org/book/bibliography.html#sproat2001nor" id="id5">(Sproat et al, 2001)</a>
...
programming languages, including Python, see <a href="https://www.nltk.org/book/bibliography.html#friedl2002mre" id="id6">(Friedl, 2002)</a>.
Other presentations include Section 2.1 of <a href="https://www.nltk.org/book/bibliography.html#jurafskymartin2008" id="id7">(Jurafsky & Martin, 2008)</a>,
and Chapter 3 of <a href="https://www.nltk.org/book/bibliography.html#mertz2003tpp" id="id8">(Mertz, 2003)</a>.
...
follows <a href="https://www.nltk.org/book/bibliography.html#brent1995" id="id9">(Brent, 1995)</a>; this work falls in the area of
language acquisition <a href="https://www.nltk.org/book/bibliography.html#niyogi2006" id="id10">(Niyogi, 2006)</a>.

Collocations are a special case of multiword expressions.
A <a id="multiword_expression_index_term"></a>multiword expression is a small phrase whose meaning
and other properties cannot be predicted from its words alone,
e.g. *part of speech* <a href="https://www.nltk.org/book/bibliography.html#baldwinkim2010" id="id11">(Baldwin & Kim, 2010)</a>.
...
patterns like *x and other ys* is described by <a href="https://www.nltk.org/book/bibliography.html#hearst1992hyp" id="id12">(Hearst, 1992)</a>.
...
1. ☼ We can use the slice notation to remove morphological endings on
...
1. ☼ We saw how we can generate an `IndexError` by indexing beyond the end
   of a string. Is it possible to construct an index that goes too far to
   the left, before the start of the string?

1. ☼ We can specify a "step" size for the slice. The following
...
1. ☼ What happens if you ask the interpreter to evaluate `monty[::-1]`?
...
1. ☼ Write regular expressions to match the following classes of strings:
   ...
   > 1. A single determiner (assume that *a*, *an*, and *the*
   >    are the only determiners).
   > 1. An arithmetic expression using integers, addition, and
   >    multiplication, such as `2*3+8`.

1. ☼ Write a utility function that takes a URL as its argument, and returns
...
1. ☼
   ...
   1. Use `nltk.regexp_tokenize()` to create a tokenizer that tokenizes
...
1. ☼ Rewrite the following loop as a list comprehension:
...
1. ☼ Define a string `raw` containing a sentence of your own choosing.
   Now, split `raw` on some character other than space, such as `'s'`.

1. ☼ Write a `for` loop to print out the characters of a string, one per line.

1. ☼ What is the difference between calling `split` on a string
...
1. ☼ Create a variable `words` containing a list of words.
   Experiment with `words.sort()` and `sorted(words)`.
   What is the difference?

1. ☼ Explore the difference between strings and integers by typing
   the following at a Python prompt: `"3" * 7` and `3 * 7`.
   Try converting between strings and integers using
   `int("3")` and `str(3)`.

1. ☼ Use a text editor to create a file
...
1. ☼ What happens when the formatting strings `%6s` and `%-6s`
   are used to display strings that are longer than six characters?

1. ◑ Read in some text from a corpus, tokenize it, and print the list of
...
1. ◑ Create a file consisting of words and (made up) frequencies, where each
...
1. ◑ Write code to access a favorite webpage and extract some text from it.
   For example, access a weather site and extract the forecast top
   temperature for your town or city today.

1. ◑ Write a function `unknown()` that takes a URL as its argument,
...
1. ◑ Examine the results of processing the URL
...
1. ◑ Are you able to write a regular expression to tokenize text in such
   a way that the word *don't* is tokenized into *do* and *n't*?
   Explain why this regular expression won't work: «`n't|\w+`».

1. ◑ Try to write code to convert text into *hAck3r*, using regular expressions
...
1. ◑ *Pig Latin* is a simple transformation of English text. Each word of the
   ...
   *idle* → *idleay*. `http://en.wikipedia.org/wiki/Pig_Latin`
   ...
   1. Write code that converts text, instead of individual words.
   1. Extend it further to preserve capitalization, to keep `qu` together
      (i.e. so that `quiet` becomes `ietquay`), and to detect when `y`
      is used as a consonant (e.g. `yellow`) vs a vowel (e.g. `style`).

1. ◑ Download some text from a language that has vowel harmony (e.g. Hungarian),
   extract the vowel sequences of words, and create a vowel bigram table.

1. ◑ Python's `random` module includes a function `choice()` which
...
1. ◑ Consider the numeric expressions in the following sentence from
...
1. ◑ Readability measures are used to score the reading difficulty of a
...
1. ◑ Use the Porter Stemmer to normalize some tokenized text, calling
   the stemmer on each word. Do the same thing with the Lancaster Stemmer
   and see if you observe any differences.

1. ◑ Define the variable `saying` to contain the list
   `['After', 'all', 'is', 'said', 'and', 'done', ',', 'more', 'is', 'said', 'than', 'done', '.']`. Process this list using a `for` loop, and store the
...
1. ◑ Define a variable `silly` to contain the string:
   `'newly formed bland ideas are inexpressible in an infuriating way'`. (This happens to be the legitimate interpretation that
   ...
   1. Split `silly` into a list of strings, one per
      word, using Python's `split()` operation, and save
      this to a variable called `bland`.
   1. Extract the second letter of each word in `silly` and join
      them into a string, to get `'eoldrnnnna'`.
   1. Combine the words in `bland` back into a single string, using `join()`.
      Make sure the words in the resulting string are separated with
      whitespace.
   1. Print the words of `silly` in alphabetical order, one per line.

1. ◑ The `index()` function can be used to look up items in sequences.
   ...
   1. Define a variable `words` containing a list of words. Now use `words.index()`
      to look up the position of an individual word.
   1. Define a variable `silly` as in the exercise above.
...
1. ◑ Write code to convert nationality adjectives like *Canadian* and
...
1. ◑ Read the LanguageLog post on phrases of the form *as best as p can*
   ...
   [3.5](#sec-useful-applications-of-regular-expressions).
   `http://itre.cis.upenn.edu/~myl/languagelog/archives/002733.html`

1. ◑ Study the lolcat version of the book of Genesis,
...
1. ◑ Read about the `re.sub()` function for string substitution
...
1. ★ An interesting challenge for tokenization is words that have been
   ...
   1. Write a regular expression that identifies words that are
      ...
      `\n` character.
   1. Use `re.sub()` to remove the `\n` character from these
      words.
   1. How might you identify words that should not remain hyphenated
      once the newline is removed, e.g. `'encyclo-\npedia'`?x

1. ★ Read the Wikipedia entry on *Soundex*. Implement this
   algorithm in Python.

1. ★ Obtain raw texts from two or more genres and compute their respective
...
1. ★ Rewrite the following nested loop as a nested list comprehension:
...
1. ★ Use WordNet to create a semantic index for a text collection.
   Extend the concordance search program in [3.6](#code-stemmer-indexing),
...
1. ★ With the help of a multilingual corpus such as the
...
1. ★ Write a program that processes a text and discovers
...
1. ★ Read the article on normalization of non-standard words
   <a href="https://www.nltk.org/book/bibliography.html#sproat2001nor" id="id13">(Sproat et al, 2001)</a>, and implement a similar system for text normalization.
