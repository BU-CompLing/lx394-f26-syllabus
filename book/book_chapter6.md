...
<a id="chap-data-intensive"></a>

Detecting patterns is a central part of Natural Language Processing. Words ending in
*-ed* tend to be past tense verbs ([5.](https://www.nltk.org/book/ch05.html#chap-tag)). Frequent use of
*will* is indicative of news text ([3](https://www.nltk.org/book/ch03.html#chap-words)). These observable
...
1. How can we identify particular features of language data that
   are salient for classifying it?
1. How can we construct models of language that can
...
<a id="sec-supervised-classification"></a>

## 1 Supervised Classification

<a id="classification_index_term"></a>Classification is the task of choosing the correct <a id="class_label_index_term"></a>class
...
A classifier is called <a id="supervised_index_term"></a>supervised if it is built based on
training corpora containing the correct label for each input. The
framework used by supervised classification is shown in
[1.1](#fig-supervised-classification).

<a id="fig-supervised-classification"></a>

![../images/supervised-classification.png](https://www.nltk.org/images/supervised-classification.png){: style="width: 456.5px; height: 235.5px;" }
...
In [4](https://www.nltk.org/book/ch02.html#sec-lexical-resources) we saw that male and female names
...
<a id="features_index_term"></a>features of the input are relevant, and how to <a id="encode_index_term"></a>encode
those features. For this example, we'll start by just looking at the
final letter of a given name. The following <a id="feature_extractor_index_term"></a>feature extractor
...
The returned dictionary, known as a <a id="feature_set_index_term"></a>feature set, maps from
...
divide the resulting list of feature sets into a <a id="training_set_index_term"></a>training set
...
ratios are known as <a id="likelihood_ratios_index_term"></a>likelihood ratios, and can be useful for
...
[1.2](#code-gender-features-overfitting).

<a id="code-gender-features-overfitting"></a>
...
*[**Example 1.2 (code_gender_features_overfitting.py)**](https://www.nltk.org/book/pylisting/code_gender_features_overfitting.py): **Figure 1.2**: A Feature Extractor that Overfits Gender Features.
...
well to new examples. This problem is known as <a id="overfitting_index_term"></a>overfitting, and
...
extractor shown in [1.2](#code-gender-features-overfitting), it will overfit
...
method for refining the feature set is <a id="error_analysis_index_term"></a>error analysis. First,
we select a <a id="development_set_index_term"></a>development set, containing the corpus data for
...
into different subsets is shown in [1.3](#fig-corpus-org).

<a id="fig-corpus-org"></a>

![../images/corpus-org.png](https://www.nltk.org/images/corpus-org.png){: style="width: 319.0px; height: 166.0px;" }
...
using the training set <a href="#err-analysis-train" id="ref-err-analysis-train"><span><span>[1]</span></span></a>, and then run it on the
dev-test set <a href="#err-analysis-run" id="ref-err-analysis-run"><span><span>[2]</span></span></a>.
...
In [1](https://www.nltk.org/book/ch02.html#sec-extracting-text-from-corpora), we saw several examples of
...
([1.4](#code-document-classify-fd)). For document topic identification, we can
...
<a href="#document-classify-all-words" id="ref-document-classify-all-words"><span><span>[1]</span></span></a>. We can then define a feature extractor
<a href="#document-classify-extractor" id="ref-document-classify-extractor"><span><span>[2]</span></span></a> that simply checks whether each of these
words is present in a given document.

<a id="code-document-classify-fd"></a><a id="document-classify-all-words"></a><a id="document-classify-extractor"></a><a id="document-classify-set"></a>
...
*[**Example 1.4 (code_document_classify_fd.py)**](https://www.nltk.org/book/pylisting/code_document_classify_fd.py): **Figure 1.4**: A feature extractor for document classification, whose
...
> <a href="#document-classify-set" id="ref-document-classify-set"><span><span>[3]</span></span></a>, rather than just checking if
> `word in document`, is
> that checking whether a word occurs in a set is much faster than
> checking whether it occurs in a list ([4.7](https://www.nltk.org/book/ch04.html#sec-algorithm-design)).
{: .note }

Now that we've defined our feature extractor, we can use it to train a
classifier to label new movie reviews ([1.5](#code-document-classify-use)). To
check how reliable the resulting classifier is, we compute its
accuracy on the test set <a href="#document-classify-test" id="ref-document-classify-test"><span><span>[1]</span></span></a>. And once again,
we can use `show_most_informative_features()` to find out which
features the classifier found to be most informative
<a href="#document-classify-smif" id="ref-document-classify-smif"><span><span>[2]</span></span></a>.

<a id="code-document-classify-use"></a><a id="document-classify-test"></a><a id="document-classify-smif"></a>
...
*[**Example 1.5 (code_document_classify_use.py)**](https://www.nltk.org/book/pylisting/code_document_classify_use.py): **Figure 1.5**: Training and testing a classifier for document classification.*
...
In [5.](https://www.nltk.org/book/ch05.html#chap-tag) we built a regular expression tagger that chooses a
...
This approach is demonstrated in [1.6](#code-suffix-pos-tag), which employs a
context-dependent feature extractor to define a part of speech tag
classifier.

<a id="code-suffix-pos-tag"></a><a id="suffix-pos-tag-fd"></a>
...
*[**Example 1.6 (code_suffix_pos_tag.py)**](https://www.nltk.org/book/pylisting/code_suffix_pos_tag.py): **Figure 1.6**: A part-of-speech classifier whose feature detector
...
tasks, we can use <a id="joint_classifier_index_term"></a>joint classifier models, which choose an
appropriate labeling for a collection of related inputs. In the case
of part-of-speech tagging, a variety of different <a id="sequence_classifier_index_term"></a>sequence
classifier models can be used to jointly choose part-of-speech
tags for all the words in a given sentence.

One sequence classification strategy, known as <a id="consecutive_classification_index_term"></a>consecutive
classification or <a id="greedy_sequence_classification_index_term"></a>greedy sequence classification, is to
...
tagger from [5](https://www.nltk.org/book/ch05.html#sec-n-gram-tagging), which began by choosing a
...
This strategy is demonstrated in [1.7](#code-consecutive-pos-tagger).
First, we must
...
the sentence so far <a href="#consec-pos-tag-features" id="ref-consec-pos-tag-features"><span><span>[1]</span></span></a>.
...
sequence classifier <a href="#consec-pos-tagger" id="ref-consec-pos-tagger"><span><span>[2]</span></span></a>. During training, we use
...
<a id="code-consecutive-pos-tagger"></a><a id="consec-pos-tag-features"></a><a id="consec-pos-tagger"></a>
...
*[**Example 1.7 (code_consecutive_pos_tagger.py)**](https://www.nltk.org/book/pylisting/code_consecutive_pos_tagger.py): **Figure 1.7**: Part of Speech Tagging with a Consecutive Classifier*
...
described in [(1)](https://www.nltk.org/book/ch05.html#sec-transformation-based-tagging), is a good example of this strategy.
...
<a id="hidden_markov_models_index_term"></a>Hidden Markov Models.
...
restriction, it is possible to use dynamic programming ([4.7](https://www.nltk.org/book/ch04.html#sec-algorithm-design))
...
two more advanced models, called <a id="maximum_entropy_markov_models_index_term"></a>Maximum Entropy Markov Models and
<a id="linear_chain_conditional_random_field_models_index_term"></a>Linear-Chain Conditional Random Field Models;
but different algorithms are used to find scores for tag sequences.

<a id="sec-further-examples-of-supervised-classification"></a>
...
in [2.1](#code-classification-based-segmenter) shows how this can be done.

<a id="code-classification-based-segmenter"></a>
...
*[**Example 2.1 (code_classification_based_segmenter.py)**](https://www.nltk.org/book/pylisting/code_classification_based_segmenter.py): **Figure 2.1**: Classification Based Sentence Segmenter*
...
<a id="dialogue_acts_index_term"></a>dialogue acts underlying the utterances in a dialogue can be an
important first step in understanding the conversation.

The NPS Chat Corpus, which was demonstrated in
[1](https://www.nltk.org/book/ch02.html#sec-extracting-text-from-corpora), consists of over 10,000 posts from
...
"hypothesis" (as already discussed in [5](https://www.nltk.org/book/ch01.html#sec-automatic-natural-language-understanding)).
...
In our RTE feature detector ([2.2](#code-rte-features)), we let words
...
*[**Example 2.2 (code_rte_features.py)**](https://www.nltk.org/book/pylisting/code_rte_features.py): **Figure 2.2**: "Recognizing Text Entailment" Feature Extractor. The
...
the labels that it generates for the inputs in a <a id="test_set_index_term_2"></a>test set
(or <a id="evaluation_set_index_term"></a>evaluation set)
...
<a id="accuracy_index_term"></a>accuracy, measures the percentage of inputs in the test
...
[2](https://www.nltk.org/book/ch11.html#sec-life-cycle-of-a-corpus).)
...
<a id="fig-precision-recall"></a>

![../images/precision-recall.png](https://www.nltk.org/images/precision-recall.png){: style="width: 523.0px; height: 283.5px;" }
...
categories shown in [3.1](#fig-precision-recall):

- <a id="true_positives_index_term"></a>True positives are relevant items that we correctly identified
  as relevant.
- <a id="true_negatives_index_term"></a>True negatives are irrelevant items that we correctly identified
  as irrelevant.
- <a id="false_positives_index_term"></a>False positives (or <a id="type_i_errors_index_term"></a>Type I errors) are irrelevant items
  that we incorrectly identified as relevant.
- <a id="false_negatives_index_term"></a>False negatives (or <a id="type_ii_errors_index_term"></a>Type II errors) are relevant items
  that we incorrectly identified as irrelevant.

Given these four numbers, we can define the following metrics:

- <a id="precision_index_term"></a>Precision, which indicates how many of the items that we
  identified were relevant, is TP/(TP+FP).
- <a id="recall_index_term"></a>Recall, which indicates how many of the relevant items that we
  identified, is TP/(TP+FN).
- The <a id="f_measure_index_term"></a>F-Measure (or <a id="f_score_index_term"></a>F-Score), which combines the precision
...
which types of mistake it made. A <a id="confusion_matrix_index_term"></a>confusion matrix is a table
where each cell [i,j] indicates how often label j was
predicted when the correct label was i. Thus, the diagonal
entries (i.e., cells <a href="#id17" id="id18"><span class="problematic">|ii|</span></a>) indicate labels that were
...
evaluations, a technique known as <a id="cross_validation_index_term"></a>cross-validation. In
particular, we subdivide the original corpus into N
subsets called <a id="folds_index_term"></a>folds. For each of these folds, we train a model using all
...
A <a id="decision_tree_index_term"></a>decision tree is a simple flowchart that selects
labels for input values. This flowchart consists of <a id="decision_nodes_index_term"></a>decision
nodes, which check feature values, and <a id="leaf_nodes_index_term"></a>leaf nodes, which
assign labels. To choose the label for an input value, we begin at
the flowchart's initial decision node, known as its <a id="root_node_index_term"></a>root node.
...
value. [4.1](#fig-decision-tree) shows an example decision tree model for
the name gender task.

<a id="fig-decision-tree"></a>

![../images/decision-tree.png](https://www.nltk.org/images/decision-tree.png){: style="width: 197.5px; height: 93.0px;" }
...
best "decision stump" for a corpus. A <a id="decision_stump_index_term"></a>decision stump is a
...
could grow the decision tree in [4.1](#fig-decision-tree) by replacing the
...
popular alternative, called <a id="information_gain_index_term"></a>information gain, measures how
...
$$H = -\sum_{l \in \text{labels}} P(l) \times \log_2 P(l). \tag{1}$$

<a id="fig-entropy"></a>

![../images/Binary_entropy_plot.png](https://www.nltk.org/images/Binary_entropy_plot.png){: style="width: 85.0px; height: 85.5px;" }
...
For example, [4.2](#fig-entropy) shows how the entropy of labels in
...
[4.3](#code-entropy) demonstrates how to calculate the entropy
of a list of labels.
...
*[**Example 4.3 (code_entropy.py)**](https://www.nltk.org/book/pylisting/code_entropy.py): **Figure 4.3**: Calculating the Entropy of a List of Labels*
...
<a id="overfit_index_term"></a>overfit the training set, learning patterns that reflect
...
<a id="prune_index_term"></a>prune decision nodes that do not improve performance on a
dev-test.
...
In <a id="naive_bayes_index_term"></a>naive Bayes classifiers, every feature gets a say in
...
by calculating the <a id="prior_probability_index_term"></a>prior probability of each label, which is
...
input value. [5.1](#fig-naive-bayes-triangle) illustrates this process.

<a id="fig-naive-bayes-triangle"></a>

![../images/naive-bayes-triangle.png](https://www.nltk.org/images/naive-bayes-triangle.png){: style="width: 370.2px; height: 237.6px;" }
...
process is illustrated in [5.2](#fig-naive-bayes-bargraph) and
[5.3](#fig-naive-bayes-graph).

<a id="fig-naive-bayes-bargraph"></a>

![../images/naive_bayes_bargraph.png](https://www.nltk.org/images/naive_bayes_bargraph.png){: style="width: 660.0px; height: 367.5px;" }
...
<a id="naive_bayes_assumption_index_term"></a>naive Bayes assumption (or <a id="independence_assumption_index_term"></a>independence assumption)
makes it much
...
<a id="fig-naive-bayes-graph"></a>

![../images/naive_bayes_graph.png](https://www.nltk.org/images/naive_bayes_graph.png){: style="width: 266.4px; height: 163.79999999999998px;" }

***Figure 5.3**: A <a id="bayesian_network_graph_index_term"></a>Bayesian Network Graph illustrating the generative process
...
$$ P(\text{label} \mathrel{|} \text{features}) = \frac{P(\text{features}, \text{label})}{P(\text{features})} \tag{2}$$
...
>
> $$ P(\text{features}) = \sum_{l \in \text{labels}} P(\text{features}, \text{label}) \tag{3}$$
{: .note }
...
$$ P(\text{features}, \text{label}) = P(\text{label}) \times P(\text{features} \mathrel{|} \text{label}) \tag{4} $$
...
$$ P(\text{features}, \text{label}) = P(\text{label}) \times \prod_{f \in \text{features}} P(f \mathrel{|} \text{label}) \tag{5} $$
...
$$ P(f \mathrel{|} \text{label}) = \frac{\text{count}(f, \text{label})}{\text{count}(\text{label})} \tag{6}$$
...
more sophisticated techniques, known as <a id="smoothing_index_term"></a>smoothing techniques,
for calculating P(f|label), the probability of a feature given a
label. For example, the <a id="expected_likelihood_estimation_index_term"></a>Expected Likelihood Estimation for the
probability of a feature given a label basically adds 0.5 to each
count(f,label) value, and the <a id="heldout_estimation_index_term"></a>Heldout Estimation uses a heldout
...
converted to binary features by <a id="binning_index_term"></a>binning, which replaces them with
...
$$ P(\text{features}, \text{label}) = w[\text{label}] \times \prod_{f \in \text{features}} w[f, \text{label}] \tag{7} $$
...
and w[f, label] the <a id="parameters_index_term"></a>parameters or <a id="weights_index_term"></a>weights for the
model. Using the naive Bayes algorithm, we set each of these
parameters independently:

$$ w[\text{label}] = P(\text{label}) \tag{8} $$

$$ w[f, \text{label}] = P(f \mathrel{|} \text{label}) \tag{9} $$
...
The <a id="maximum_entropy_index_term"></a>Maximum Entropy classifier uses a model that is very
...
parameters that maximizes the <a id="total_likelihood_index_term"></a>total likelihood of the training
corpus, which is defined as:

$$ P(\text{features}) = \sum_{x \in \text{corpus}} P(\text{label}(x) \mathrel{|} \text{features}(x)) \tag{10} $$
...
$$ P(\text{label} \mathrel{|} \text{features}) = \frac{P(\text{label}, \text{features})}{\sum_{\text{label}} P(\text{label}, \text{features})} \tag{11} $$
...
using <a id="iterative_optimization_index_term"></a>iterative optimization techniques, which initialize the
...
parameter is called a <a id="joint_feature_index_term"></a>joint-feature. Note that joint-features
...
$$ P(\text{input}, \text{label}) = \prod_{\text{joint-features}(\text{input}, \text{label})} w[\text{joint-feature}] \tag{12} $$
...
general, the <a id="maximum_entropy_principle_index_term"></a>Maximum Entropy principle states that, among the
...
<a id="generative_index_term"></a>generative classifier, which builds a model that predicts
...
1. What is the most likely input value?
1. How likely is a given input value?
1. How likely is a given input value with a given label?
1. What is the most likely label for an input that might have one
   of two values (but we don't know which)?

The Maximum Entropy classifier, on the other hand, is an example of a
<a id="conditional_index_term"></a>conditional classifier. Conditional classifiers build models
...
<a id="models_index_term"></a>models that capture those patterns. Typically, these models are
...
But all explicit models can make predictions about new "<a id="unseen_index_term"></a>unseen"
...
contains those patterns. For example, as we saw in [3.1](https://www.nltk.org/book/ch03.html#tab-absolutely),
...
<a href="https://www.nltk.org/book/bibliography.html#alpaydin2004" id="id1">(Alpaydin, 2004)</a>. For a more mathematically intense introduction to
the theory of machine learning, see <a href="https://www.nltk.org/book/bibliography.html#hastie2009" id="id2">(Hastie, Tibshirani, & Friedman, 2009)</a>. Excellent books on
using machine learning techniques for NLP include <a href="https://www.nltk.org/book/bibliography.html#abney2008" id="id3">(Abney, 2008)</a>,
<a href="https://www.nltk.org/book/bibliography.html#daelemans2005" id="id4">(Daelemans & Bosch, 2005)</a>, <a href="https://www.nltk.org/book/bibliography.html#feldman2007" id="id5">(Feldman & Sanger, 2007)</a>, <a href="https://www.nltk.org/book/bibliography.html#segaran2007" id="id6">(Segaran, 2007)</a>, <a href="https://www.nltk.org/book/bibliography.html#weiss2004" id="id7">(Weiss et al, 2004)</a>. For
more on smoothing techniques for language problems, see
<a href="https://www.nltk.org/book/bibliography.html#manning1999fsn" id="id8">(Manning & Schutze, 1999)</a>. For more on sequence modeling, and especially
hidden Markov models, see <a href="https://www.nltk.org/book/bibliography.html#manning1999fsn" id="id9">(Manning & Schutze, 1999)</a> or <a href="https://www.nltk.org/book/bibliography.html#jurafskymartin2008" id="id10">(Jurafsky & Martin, 2008)</a>.
Chapter 13 of <a href="https://www.nltk.org/book/bibliography.html#manning2008ir" id="id11">(Manning, Raghavan, & Schutze, 2008)</a> discusses the use of naive Bayes for
classifying texts.
...
of numerically intensive algorithms in Python, see <a href="https://www.nltk.org/book/bibliography.html#kiusalaas2005" id="id12">(Kiusalaas, 2005)</a>.

The classification techniques described in this chapter can be applied
to a very wide variety of problems. For example, <a href="https://www.nltk.org/book/bibliography.html#agirre2007" id="id13">(Agirre & Edmonds, 2007)</a> uses
classifiers to perform word-sense disambiguation; and <a href="https://www.nltk.org/book/bibliography.html#melamed2001" id="id14">(Melamed, 2001)</a>
uses classifiers to create parallel texts. Recent textbooks that
cover text classification include <a href="https://www.nltk.org/book/bibliography.html#manning2008ir" id="id15">(Manning, Raghavan, & Schutze, 2008)</a> and <a href="https://www.nltk.org/book/bibliography.html#croft2009" id="id16">(Croft, Metzler, & Strohman, 2009)</a>.
...
1. ☼ Using any of the three classifiers described in this
...
1. ☼ The Senseval 2 Corpus contains data intended to train
...
1. ◑
   ...
   [1.7](#code-consecutive-pos-tagger) to get some ideas.
...
1. ◑
   ...
   training data. One solution is to make use of a <a id="lexicon_index_term"></a>lexicon,
...
1. ★ Suppose you wanted to automatically generate a prose description of a scene,
...
System Message: ERROR/3 (`ch06.rst2`, line 1264); *[backlink](#id18)*
