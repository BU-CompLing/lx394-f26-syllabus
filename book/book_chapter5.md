...
1. What is a good Python data structure for storing words and their categories?
1. How can we automatically tag each word of a text with its word class?
...
The process of classifying words into their <a id="parts_of_speech_index_term"></a>parts of speech and
labeling them accordingly is known as <a id="part_of_speech_tagging_index_term"></a>part-of-speech tagging,
<a id="pos_tagging_index_term"></a>POS-tagging, or simply <a id="tagging_index_term"></a>tagging. Parts of speech
are also known as <a id="word_classes_index_term"></a>word classes or <a id="lexical_categories_index_term"></a>lexical categories.
The collection of tags
used for a particular task is known as a <a id="tagset_index_term"></a>tagset. Our emphasis
in this chapter is on exploiting tags, and tagging text automatically.

<a id="sec-using-a-tagger"></a>
...
Several of the corpora included with NLTK have been <a id="tagged_index_term"></a>tagged for
...
For example, [2.1](#fig-tag-indian) shows data accessed using
`nltk.corpus.indian`.

<a id="fig-tag-indian"></a>

![../images/tag-indian.png](https://www.nltk.org/images/tag-indian.png){: style="width: 800.4px; height: 213.0px;" }
...
(shown in [2.1](#tab-universal-tagset)).

<a id="tab-universal-tagset"></a>
...
verb, as shown in [2.2](#tab-syntax-nouns).

<a id="tab-syntax-nouns"></a>
...
Two other important word classes are <a id="adjectives_index_term"></a>adjectives and <a id="adverbs_index_term"></a>adverbs.
...
prepositions, such as <a id="articles_index_term"></a>articles (also often called <a id="determiners_index_term"></a>determiners)
(e.g., *the*, *a*), <a id="modals_index_term"></a>modals (e.g., *should*,
*may*), and <a id="personal_pronouns_index_term"></a>personal pronouns (e.g., *she*, *they*).
...
The program in [2.2](#code-findtags) finds all tags starting with `NN`,
...
*[**Example 2.2 (code_findtags.py)**](https://www.nltk.org/book/pylisting/code_findtags.py): **Figure 2.2**: Program to Find the Most Frequent Noun Tags*
...
In code-three-word-phrase we consider each three-word window in the sentence <a href="#three-word" id="ref-three-word"><span><span>[1]</span></span></a>,
and check if they meet our criterion <a href="#verb-to-verb" id="ref-verb-to-verb"><span><span>[2]</span></span></a>. If the tags
match, we print the corresponding words <a href="#print-words" id="ref-print-words"><span><span>[3]</span></span></a>.

<a id="code-three-word-phrase"></a><a id="three-word"></a><a id="verb-to-verb"></a><a id="print-words"></a>
...
*[**Example 2.3 (code_three_word_phrase.py)**](https://www.nltk.org/book/pylisting/code_three_word_phrase.py): **Figure 2.3**: Searching for Three-Word Phrases Using POS Tags*
...
<a id="mapping_index_term"></a>mapping from words to tags. The most natural way to
store mappings in Python uses the so-called <a id="dictionary_index_term"></a>dictionary data type
(also known as an <a id="associative_array_index_term"></a>associative array or <a id="hash_array_index_term"></a>hash array
...
kind of table, as shown in [3.1](#fig-maps01).

<a id="fig-maps01"></a>

![../images/maps01.png](https://www.nltk.org/images/maps01.png){: style="width: 136.8px; height: 113.4px;" }

***Figure 3.1**: List Look-up: we access the contents of a Python list with the help of an integer index.*

Contrast this situation with frequency distributions ([3](https://www.nltk.org/book/ch01.html#sec-computing-with-language-simple-statistics)),
...
![../images/maps02.png](https://www.nltk.org/images/maps02.png){: style="width: 719.62px; height: 170.5px;" }
...
arbitrary types of information. [3.1](#tab-linguistic-objects) lists a variety
of linguistic objects, along with what they map.

<a id="tab-linguistic-objects"></a>
...
Python provides a <a id="dictionary_index_term_2"></a>dictionary data type that can be used for
...
as we see from [3.1](#tab-linguistic-objects), it has a much wider range of uses.
...
So, for example, <a href="#pos-colorless" id="ref-pos-colorless"><span><span>[1]</span></span></a> says that
the part-of-speech of *colorless* is adjective, or more
specifically, that the <a id="key_index_term"></a>key `'colorless'`
is assigned the <a id="value_index_term"></a>value `'ADJ'` in dictionary `pos`.
When we inspect the value of `pos` <a href="#pos-inspect" id="ref-pos-inspect"><span><span>[2]</span></span></a> we see
...
variable `pos`. As we saw above (line [[2]](#pos-inspect)), this gives
...
but mappings (cf. [3.2](#fig-maps02)), and the keys are not inherently
ordered.

Alternatively, to just find the keys, we can convert the
dictionary to a list <a href="#dict-to-list" id="ref-dict-to-list"><span><span>[1]</span></span></a> — or use
the dictionary in a context where a list is expected,
as the parameter of `sorted()` <a href="#dict-sorted" id="ref-dict-sorted"><span><span>[2]</span></span></a>,
or in a `for` loop <a href="#dict-for-loop" id="ref-dict-for-loop"><span><span>[3]</span></span></a>.
...
We can even sort tuples <a href="#sort-tuples" id="ref-sort-tuples"><span><span>[1]</span></span></a>, which orders them according to their first element
...
saw in [4](https://www.nltk.org/book/ch02.html#sec-lexical-resources) for the CMU Pronouncing Dictionary,
...
whose default value for any entry is `'N'` <a href="#default-noun" id="ref-default-noun"><span><span>[1]</span></span></a>.
When we access a non-existent entry <a href="#non-existent" id="ref-non-existent"><span><span>[2]</span></span></a>,
it is automatically added to the dictionary <a href="#automatically-added" id="ref-automatically-added"><span><span>[3]</span></span></a>.
...
> [4.4](https://www.nltk.org/book/ch04.html#sec-functions). This lambda expression specifies no
...
for tallying words shown in [fig-tally](https://www.nltk.org/book/ch01.html#fig-tally).
...
*[**Example 3.3 (code_dictionary.py)**](https://www.nltk.org/book/pylisting/code_dictionary.py): **Figure 3.3**: Incrementally Updating a Dictionary, and Sorting by Value*

The listing in [3.3](#code-dictionary) illustrates an important idiom for
...
[3.3](#code-dictionary), where we initialize a `defaultdict` and then use a
...
corpus, processing a pair of word-tag pairs for each iteration <a href="#processing-pairs" id="ref-processing-pairs"><span><span>[1]</span></span></a>.
Each time through the loop we updated our `pos` dictionary's
entry for `(t1, w2)`, a tag and its *following* word <a href="#tag-word-update" id="ref-tag-word-update"><span><span>[2]</span></span></a>.
When we look up an item in `pos` we must specify a compound key <a href="#compound-key" id="ref-compound-key"><span><span>[3]</span></span></a>,
...
A summary of Python's dictionary methods is given in [3.2](#tab-dict).
...
> regular expression tagger. (Note that [1](https://www.nltk.org/book/ch06.html#sec-supervised-classification)
...
a process known as <a id="backoff_index_term"></a>backoff ([5](#sec-n-gram-tagging)).
...
evaluate lookup taggers having a range of sizes, in [4.1](#code-baseline-tagger).

<a id="code-baseline-tagger"></a>
...
*[**Example 4.1 (code_baseline_tagger.py)**](https://www.nltk.org/book/pylisting/code_baseline_tagger.py): **Figure 4.1**: Lookup Tagger Performance with Varying Model Size*

<a id="fig-tag-lookup"></a>

![../images/tag-lookup.png](https://www.nltk.org/images/tag-lookup.png){: style="width: 490.40000000000003px; height: 370.40000000000003px;" }
...
in [4.8](https://www.nltk.org/book/ch04.html#sec-libraries).)
...
pipeline in [fig-sds](https://www.nltk.org/book/ch01.html#fig-sds); any errors in the output of one
...
<a id="gold_standard_index_term"></a>gold standard test data. This is a corpus which has been manually
...
> the challenges of creating a corpus in [11.](https://www.nltk.org/book/ch11.html#chap-data).
{: .note }

<a id="sec-n-gram-tagging"></a>
...
A unigram tagger behaves just like a lookup tagger ([4](#sec-automatic-tagging)),
...
We <a id="train_index_term"></a>train a `UnigramTagger` by specifying tagged sentence data as
...
An <a id="n_gram_tagger_index_term"></a>n-gram tagger is a generalization of a unigram tagger whose context is
the current word together with the part-of-speech tags of the
*n*-1 preceding tokens, as shown in [5.1](#fig-tag-context). The tag to be
chosen, *t*n, is circled, and the context is shaded
in grey. In the example of an n-gram tagger shown in [5.1](#fig-tag-context),
...
<a id="fig-tag-context"></a>

![../images/tag-context.png](https://www.nltk.org/images/tag-context.png){: style="width: 542.4px; height: 162.4px;" }
...
this is related to the <a id="precision_recall_trade_off_index_term"></a>precision/recall trade-off in information
retrieval).
...
> the start of a sentence, *t*n-1 and preceding
...
1. If the bigram tagger is unable to find a tag for the token, try
   the unigram tagger.
1. If the unigram tagger is also unable to find a tag, use a default tagger.
...
How can we do better with these unknown words, or <a id="out_of_vocabulary_index_term"></a>out-of-vocabulary items?
...
with a special word *UNK* using the method shown in [3](#sec-dictionaries).
...
ambiguity. In other cases however, as noted by <a href="https://www.nltk.org/book/bibliography.html#abney1996pst" id="id1">(Church, Young, & Bloothooft, 1996)</a>, the
...
linguistic knowledge. Can this idea be pushed further? In [7.](https://www.nltk.org/book/ch07.html#chap-chunk),
we shall see that it can.

<a id="sec-transformation-based-tagging"></a>
...
template of the following form: "replace *T*1 with
*T*2 in the context *C*". Typical contexts are the
...
its training phase, the tagger guesses values for *T*1,
*T*2 and *C*, to create thousands of candidate rules.
...
[6.1](#code-brill-demo) demonstrates NLTK's Brill tagger.
...
*[**Example 6.1 (code_brill_demo.py)**](https://www.nltk.org/book/pylisting/code_brill_demo.py): **Figure 6.1**: Brill Tagger Demonstration: the tagger has a collection of
...
<a id="sec-how-to-determine-the-category-of-a-word"></a>
...
<a id="present_participle_index_term"></a>present participle of a verb ends in *-ing*, and expresses
the idea of ongoing, incomplete action (e.g. *falling*, *eating*).
The *-ing* suffix also appears on nouns derived from verbs, e.g. *the
falling of the leaves* (this is known as the <a id="gerund_index_term"></a>gerund).
...
reflected in calling nouns an <a id="open_class_index_term"></a>open class. By contrast, prepositions
are regarded as a <a id="closed_class_index_term"></a>closed class. That is, there is a limited set of
...
Common tagsets often capture some <a id="morpho_syntactic_index_term"></a>morpho-syntactic information;
...
```
(4) a. *They sometimes _goes_ to the cafe.
...
```
(5) a. *All the cakes have _goes_.
    b. *He sometimes _gone_ to the cafe.
```
...
as summarized in [7.1](#tab-morphosyntax).

<a id="tab-morphosyntax"></a>
...
of <a id="morphological_analysis_index_term"></a>morphological analysis.
...
<a id="sec-tag-further-reading"></a>
...
Chapters 4 and 5 of <a href="https://www.nltk.org/book/bibliography.html#jurafskymartin2008" id="id2">(Jurafsky & Martin, 2008)</a> contain more advanced
material on n-grams and part-of-speech tagging.
The "Universal Tagset" is described by <a href="https://www.nltk.org/book/bibliography.html#petrov2011" id="id3">(Petrov, Das, & McDonald, 2012)</a>.
Other approaches to tagging involve machine learning methods ([chap-data-intensive](https://www.nltk.org/book/ch06.html#chap-data-intensive)).
In [7.](https://www.nltk.org/book/ch07.html#chap-chunk) we will see a generalization of tagging called *chunking* in which a
...
listed in [1.](https://www.nltk.org/book/ch01.html#chap-introduction).
...
1. *SemCor: Brown Corpus tagged with WordNet senses:*
...
1. *Morphological tagging, from the Turin University Italian Treebank:*
...
of dialogue act tagging, from the NPS Chat Corpus <a href="https://www.nltk.org/book/bibliography.html#forsyth2007" id="id4">(Forsyth & Martell, 2007)</a> included with
...
1. ☼ Review the mappings in [3.1](#tab-linguistic-objects). Discuss any other
...
1. ☼ Satisfy yourself that there are
   ...
   illustrated in [(3d)](#ex-go) in [7](#sec-how-to-determine-the-category-of-a-word).
1. ☼
...
1. ☼ We can use a dictionary to specify the values to be
   ...
   different formats.
1. ◑ Use `sorted()` and `set()` to get a sorted list of tags used in the Brown
   corpus, removing duplicates.
1. ◑ Write programs to process the Brown Corpus and find answers to the following
   questions:
   ...
   1. Which word has the greatest number of distinct tags. What are they, and
      what do they represent?
   1. List tags in order of decreasing frequency. What do the 20 most frequent tags represent?
   1. Which tags are nouns most commonly found after? What do these tags represent?
1. ◑ Explore the following issues that arise in connection with the lookup tagger:
   1. What happens to the tagger performance for the various
      model sizes when a backoff tagger is omitted?
   1. Consider the curve in [4.2](#fig-tag-lookup); suggest a
...
1. ◑ What is the upper limit of performance for a lookup tagger,
...
1. ◑ Generate some statistics for tagged data to answer the following questions:
   1. What proportion of word types are always assigned the same part-of-speech tag?
   1. How many words are ambiguous, in the sense that they appear with at least two tags?
   1. What percentage of word *tokens* in the Brown Corpus involve
      these ambiguous words?
1. ◑ The `evaluate()` method works out how accurately
   ...
   1. A tagger `t` takes a list of words as input, and produces a list of tagged words
   ...
   1. Once the tagger has created newly tagged text, how might the `evaluate()` method
      go about comparing it with the original tagged text and computing the accuracy score?
   1. Now examine the source code to see how the method is implemented. Inspect
...
1. ◑ Write code to search the Brown Corpus for particular words and phrases
   ...
   1. Identify words that can be plural nouns or third person singular verbs
      (e.g. *deals*, *flies*).
   1. Identify three-word prepositional phrases of the form IN + DET + NN
      (eg. *in the lab*).
   1. What is the ratio of masculine to feminine pronouns?
1. ◑ In [3.1](https://www.nltk.org/book/ch03.html#tab-absolutely) we saw a table involving frequency counts for
...
1. ◑ Obtain some tagged data for another language, and train and
...
1. ◑ [4.1](#code-baseline-tagger) plotted a curve showing
   ...
   data is varied.
1. ◑
   Inspect the confusion matrix for the bigram tagger `t2` defined in [5](#sec-n-gram-tagging),
...
1. ◑
   Modify the program in [4.1](#code-baseline-tagger) to use a logarithmic scale on
...
1. ◑ Write code that builds a dictionary of dictionaries of sets.
   Use it to store the set of POS tags that can follow a given word having
   a given POS tag, i.e. wordi → tagi
   → tagi+1.
1. ★ There are 264 distinct words in the Brown Corpus having exactly
   three possible tags.
   1. Print a table with the integers 1..10 in one column, and the
      ...
      in the other column.
   1. For the word with the greatest number of distinct tags, print
      out sentences from the corpus containing the word, one for each
      possible tag.
1. ★ Write a program to classify contexts involving the word *must* according
...
1. ★
   ...
   1. Try varying the size of the training corpus. How does it affect
      your results?
1. ★
   ...
   1. Create a new kind of unigram tagger that looks at the tag of the previous word,
   ...
   1. Add this tagger to the sequence of backoff taggers (including ordinary trigram
      and bigram taggers that look at words), right before the usual default tagger.
   1. Evaluate the contribution of this new unigram tagger.
1. ★
   Consider the code in [5](#sec-n-gram-tagging) which
   determines the upper bound for accuracy of a trigram tagger.
   Review Abney's discussion concerning the impossibility of
   exact tagging <a href="https://www.nltk.org/book/bibliography.html#abney1996pst" id="id5">(Church, Young, & Bloothooft, 1996)</a>. Explain why correct tagging of
...
1. ★
   ...
   discussed in [3](https://www.nltk.org/book/ch06.html#sec-evaluation), to improve the accuracy of the evaluations.)
1. ★
